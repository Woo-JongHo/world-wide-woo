...
Linear Project의 **Update**를 `0.0.N` 기능 릴리스 기록으로 유지한다. 이슈의 상세 진행이나 Obsidian 계약을 복제하지 않는다.
...
1. World Wide Woo Project의 최신 Update와 `package.json` 버전, 기준 commit 또는 이전 릴리스 PR을 읽는다. 완료: 다음 번호와 수집 범위가 하나로 정해진다.
2. 기준 이후 실제 diff·commit·PR·Linear 이슈를 읽어 `기능`과 `함께 묶인 변경`으로 분류한다. 기능이 없으면 Update를 게시하지 않고 대기 목록만 유지한다.
...
6. 게시 직전에 최신 Update와 head를 다시 읽고, 같은 버전 Update가 있으면 새로 만들지 않고 기존 항목을 갱신한다.
