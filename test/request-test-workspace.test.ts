...
test("projects tests as kind -> what -> goal and keeps evidence-backed outcomes separate from completed plans", () => {
	...
	expect(checks[0]).toMatchObject({ kind: "black-box", title: "실제 /model 명령으로 Www 선택", purpose: "선택·저장·다음 실행의 모델이 일치함을 증명", status: "unknown", evidenceActivityId: null });
...
test("renders a readable three-level black-box hierarchy with source on the goal line", () => {
	const snapshot = wwwFixture("ready"); snapshot.requestRuntime = [request()]; snapshot.activities = [activity("e2e", 2, "bun scripts/model-canary.ts --live", 0)];
	const projection = projectWwwTestView(snapshot);
	const output = stripTerminalSequences(new WwwTestView(() => projection).render(80).join("\n"));
...
test("keeps snapshot projection outside the render pass", () => {
	...
	expect(renderWwwTestView(projection, 80).join("\n")).toContain("Www 모델 변경을 고쳐줘");
});
