...
import { WwwInset }                                              from "../src/adapters/inbound/tui/shell/www-surface";
...
test("같은 자식 배열에서 행이 이동해도 새 순서의 바이트 출력을 반환한다", () => {
	...
	const inset = new WwwInset(new MutableRows(rows));
...
test("width와 padding 전환 및 invalidate 뒤에도 원본 바이트 출력을 유지한다", () => {
	...
	const inset  = new WwwInset(child, 2)                                            ;
...
test("보존 cache는 entry/논리 byte 상한과 마지막 generation만 유지한다", () => {
	...
	const inset    = new WwwInset(child, 0)                                                                                                                                              ;
