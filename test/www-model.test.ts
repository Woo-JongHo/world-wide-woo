...
test("Www model selections survive YAML reload without changing workload defaults", async () => {
	const root = await mkdtemp(join(tmpdir(), "www-model-")); await mkdir(join(root, ".www"));
	try {
...
test("the picker applies Www max explicitly and keeps Ultra distinct", async () => {
	let applied   : WwwSettings | undefined                                                                                                                                                                                                                                                                                                                           ;
	const current : WwwSettings = { provider: "openai-codex", model: "gpt-5.6-sol", effort: "medium" }                                                                                                                                                                                                                                                                ;
	const picker                = new ModelPickerOverlay(current, async provider => ({ state: "configured", provider, source: "fixture", type: "oauth" }), () => {}, async value => { applied = value; }, () => {}, () => {}, { ...current, model: "gpt-6-astra" }, false, { providers: ["openai-codex"], startAtModel: true, appearance: "www", nativeCodex: true }) ;
...
test("a small model sheet reveals the current effort immediately after advancing", async () => {
	...
	const picker = new ModelPickerOverlay(current, async provider => ({ state: "configured", provider, source: "fixture", type: "oauth" }), () => {}, async () => {}, () => {}, () => {}, current, false, { providers: ["openai-codex"], startAtModel: true, appearance: "www", nativeCodex: true });
	picker.start(); await Bun.sleep(0);
	const sheet = new WwwSheet(picker, () => 12); sheet.render(60); sheet.handleInput("\r");
...
test.each(["xhigh", "max", "ultra"] as Effort[])("Workbench passes Www %s to Native without aliasing or changing a running model", async effort => {
