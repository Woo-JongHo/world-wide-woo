...
function response(content: AssistantMessage["content"], stopReason: AssistantMessage["stopReason"] = "stop"): AssistantMessage {
	return { role: "assistant", content, stopReason } as AssistantMessage;
}
	...
	test("sends one stable packet-only context without tools or cwd and records structural isolation", async () => {
		...
		const models: PiDetachedCodexModels = {
			...
			streamSimple: (_model, context, options) => {
				...
				return { result: async () => response([{ type: "text", text: "요약 결과" }]) } as AssistantMessageEventStream;
			},
		};
		const signal = new AbortController().signal;
		const result = await new PiDetachedCodexGenerator(models, "gpt-5.6-sol", "2026-09-01").generate(request, signal);
