...
function denseOracle(snapshot: ReturnType<typeof history>, insetWidth: number): string[] {
	...
	for (const activity of snapshot.activities) {
		...
		if (message.role === "assistant") {
			const markdown = new Markdown(safe(message.content, 24_000), 0, 0, astraMarkdownTheme);
			...
			const markdown = new Markdown(safe(message.content, 24_000), 0, 0, astraMarkdownTheme);
			rows.push("", pair(astraTitle(labels.get(message.id)!, ink), "", transcriptWidth));
			rows.push(...markdown.render(Math.max(1, transcriptWidth - 2)).map(row => `  ${a.text(row)}`), "");
		}
	}
	return rows.map(row => fit(`${insetWidth > 8 ? "  " : ""}${fit(row, transcriptWidth)}`, insetWidth));
}

test("실제 AstraWorkspace frame은 dense oracle과 같고 viewport 범위만 요청한다", () => {
	const snapshot = history(100);
	const workspace = new AstraWorkspace(() => snapshot, () => []);
	workspace.toggleSidebar();
	const transcript = workspace.transcript as AstraTranscriptView & {
...
test("실제 AstraWorkspace frame은 새 volatile draft를 count와 paint 사이 재사용한다", () => {
...
test("빈 화면에서 첫 durable append가 생기면 안내 block을 남기지 않는다", () => {
	...
	const view = new AstraTranscriptView(empty);
...
test("실제 lazy AstraWorkspace는 폭 왕복 뒤 읽던 marker와 follow=false를 보존한다", () => {
...
test("중복 본문 anchor는 실제 dense와 lazy frame에서 같은 first-match 휴리스틱을 따른다", () => {
	...
	const lazyTranscript = new AstraTranscriptView(repeated);
	const denseTranscript = new AstraTranscriptView(repeated);
