...
describe("interpreted Plan activity and cards", () => {
	test("shows the newest five interpretations in chronological order, ignoring raw event noise", () => {
		const snapshot: WorkbenchSnapshot = { ...astraFixture(),
		...
		expect(astraNowLabel(snapshot)).toBe("검사 7의 표시 동작을 확인합니다.");
	...
	test("renders the interpreted feed through the real runtime presentation too", () => {
		const snapshot: WorkbenchSnapshot = { ...astraFixture(), planActivities: [activity(1)], planActivityStatus: "pending" };
		...
		const page = stripTerminalSequences(new AstraPlanView(() => snapshot, false, Date.now, false, presentation).render(80).join("\n"));
	...
	test("pending Plan cards keep their titles without a waiting status label", () => {
		...
		for (const compact of [false, true]) {
			const output = stripTerminalSequences(new AstraPlanView(() => snapshot, compact).render(36).join("\n"));
	...
	test("waits without raw fallbacks and replaces one interpreted action on completion", () => {
		let snapshot: WorkbenchSnapshot = { ...astraFixture(), planActivityStatus: "pending" };
		const view = new AstraPlanView(() => snapshot);
		...
		expect(running.slice(running.indexOf("Activity"))).not.toContain("진행 중");
		...
		expect(complete.slice(complete.indexOf("Activity"))).not.toContain("완료");
