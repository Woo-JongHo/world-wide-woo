...
import { FilePlanningStore } from "../src/system/adapters/planning-store.js";
...
describe("FilePlanningStore", () => {
	...
	test("serializes independent process writers without duplicate IDs", async () => {
		const root = await workspace();
		const script = `import { FilePlanningStore } from "./src/system/adapters/planning-store.ts"; await new FilePlanningStore(process.env.PLANNING_ROOT).createEpic(process.argv[1], "Goal");`;
