...
import { WwwUsageRail, WwwUsageView }           from "../src/adapters/inbound/tui/features/usage/view/www-usage-view";
...
describe("WwwUsageView", () => {
	test("shows only observed session and provider usage", () => {
		...
		const output = stripTerminalSequences(new WwwUsageView(() => snapshot, () => usage).render(120).join("\n"));
	...
	test("uses shared provider cards wide, stacks them compactly, and keeps the rail honest", () => {
		...
		const view        = new WwwUsageView(() => snapshot, () => usage) ;
		...
		const rail = stripTerminalSequences(new WwwUsageRail(() => snapshot, () => usage).render(38).join("\n"));
