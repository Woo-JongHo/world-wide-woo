...
export function wwwFixture(phase: WorkbenchSnapshot["phase"] = "working"): WorkbenchSnapshot {
	...
	const activity = (id: string, kind: ProjectActivity["kind"], state: ProjectActivity["phase"], payload: ProjectActivity["payload"], itemId = id): ProjectActivity => ({
		schemaVersion: 1, id, projectId: "www-preview", sequence: activities.length + 1, recordedAt: `2026-09-11T09:42:${String(activities.length).padStart(2, "0")}.000Z`, kind, phase: state, provider: "openai-codex", nativeRefs: { threadId: "preview-thread", turnId: "preview-turn", itemId }, sourceDigest: `sha256:${"a".repeat(64)}`, payload,
	});
	...
	return {
		projectId: "www-preview", revision: 1, journalSequence: activities.length, phase,
