...
import { WwwWorkflowRail, WwwWorkflowView }     from "../src/adapters/inbound/tui/features/workflow/view/www-workflow-view";
...
test("Workflow 데모는 Figma 관계 트리와 두 열 실행 패널을 정렬하고 live에 샘플을 섞지 않는다", () => {
	...
	const view         = new WwwWorkflowView(() => snapshot, () => true)               ;
...
test("Workflow의 160열 첫 viewport에 7단계와 역할별 위임 상태가 함께 보인다", () => {
	...
	const overflow = stripTerminalSequences(new WwwWorkflowView(() => snapshot).render(120).join("\n"));
	expect(overflow).toContain("… 1 delegated tasks hidden");
	snapshot.delegation = [{ ...projection, tasks: [{ ...template, status: "pending" }] }];
	const pending = stripTerminalSequences(new WwwWorkflowView(() => snapshot).render(120).join("\n"));
