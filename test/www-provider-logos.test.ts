...
import { WwwHud }                              from "../src/adapters/inbound/tui/shell/www-surface";
import { wwwUsageLine }                        from "../src/adapters/inbound/tui/features/usage/view/www-usage";
...
test("keeps provider names on terminals without Kitty graphics", () => {
	for (const images of [null, "iterm2"] as const) {
		...
		const row = wwwUsageLine(usage, 200, "gpt-6-astra", 1, true);
...
test("fullscreen host emits all logos, clears placements on resize and image data on stop", async () => {
	...
	let showUsage  = true                                                                                        ;
	const hud      = new WwwHud(() => ({ ...wwwFixture(), hud: { showUsage, showContext: true } }), () => usage) ;
