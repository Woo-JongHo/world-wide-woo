...
describe("WwwExecutionHeading activity projection", () => {
	test("reuses a deeply immutable activity revision and invalidation rebuilds it", () => {
		...
		const heading = new WwwExecutionHeading(() => snapshot, () => hint, () => now) ;
	...
	test("does not trust a shallow-frozen activity array with mutable records", () => {
		...
		const heading = new WwwExecutionHeading(() => snapshot, undefined, () => Date.parse("2026-09-11T09:42:20.000Z"), false);
		expect(stripTerminalSequences(heading.render(100)[0]!)).toContain("✓ 처리");

		terminal.phase = "failed";
		terminal.payload = { method: "turn/failed" };
		const failed = stripTerminalSequences(heading.render(100)[0]!);
	...
	test("active turn changes invalidate the immutable activity projection key", () => {
		...
		const heading = new WwwExecutionHeading(() => snapshot, undefined, () => Date.parse("2026-09-11T09:42:30.000Z"), false);
		snapshot.activeTurnId = "preview-turn";
		const original = stripTerminalSequences(heading.render(100)[0]!);
		snapshot.activeTurnId = "later-turn";
		const switched = stripTerminalSequences(heading.render(100)[0]!);
	...
	test("keeps the first started event when its recordedAt is invalid", () => {
		...
		const heading = new WwwExecutionHeading(() => snapshot, undefined, () => Date.parse("2026-09-11T09:42:20.000Z"), false);
		expect(stripTerminalSequences(heading.render(100)[0]!)).not.toContain("✓ 처리");

		snapshot.phase = "working";
		snapshot.activeTurnId = "preview-turn";
		expect(stripTerminalSequences(heading.render(100)[0]!)).toContain("실행 경과 계산 중");
	});
	...
	test("layout frames do not invalidate the production heading component", () => {
		const snapshot = deepFreeze(wwwFixture("ready"));
		class ObservedHeading extends WwwExecutionHeading {
			invalidations = 0;
		...
		const workspace = new WwwWorkspace(() => snapshot, () => [], undefined, Date.now, false, null, undefined, heading);
