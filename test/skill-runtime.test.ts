...
describe("RPA Agent runtime", () => {
	...
	test("Skill Run이 순서, stale 승인, evidence와 Receipt를 강제한다", () => {
		...
		let state = beginSkillStep(startSkillRun(scenario, runId));
		...
		const result = finishSkillStep(state, "succeeded", ["read-back matched"]);
	...
	test("상태 CAS와 Receipt 저장·조회를 run 경계 안에서 수행한다", async () => {
		...
		const scenario = planRpaScenario({ intent: "reconcile", registry: registry(), processId: "RPA-GMB-FTA" });
		const initial = startSkillRun(scenario, runId);
		...
		const result = finishSkillStep(running, "succeeded", ["read-back matched"]);
