...
test.each([160, 200])("Dashboard fits its lower analysis and rail in the first %i by 36 viewport", width => {
	...
	const workspace = new WwwWorkspace(() => demo.snapshot, () => demo.usage, undefined, () => 0, false, null, dashboard, undefined, undefined, {}, undefined, () => true) ;
...
test.each([100, 118, 160, 200])("Dashboard Figma catalog keeps panels and fixed meter axes within %i cells", width => {
	const rows  = new WwwDashboardView(wwwFixture, () => true).render(width) ;
...
test.each([1, 20, 40, 60, 80])("Dashboard catalog and rail remain bounded at %i cells", width => {
	const components = [new WwwDashboardView(wwwFixture, () => true), new WwwDashboardRail(wwwFixture, () => true)];
...
test("Dashboard heatmap has distinct supplied intensity colors and rail shows bounded load meters", () => {
	...
	chalk.level = 3;
	try {
		const rows = new WwwDashboardView(wwwFixture, () => true).render(160);
		...
		const rail = stripTerminalSequences(new WwwDashboardRail(wwwFixture, () => true).render(34).join("\n"));
