...
export class TNotesSourceView implements Component {
	constructor(
		private readonly getSnapshot: () => Pick<WorkbenchSnapshot, "tnotes" | "tnoteVisibleLimit" | "tnoteSummaryMaxChars" | "tnoteSummaryMaxLines">,
		private readonly getSelectedNoteId: () => string | null = () => null,
	) {}
	invalidate(): void {}
	render(width: number): string[] {
		...
		for (const [visibleIndex, note] of visibleTNotes.entries()) {
			...
			const cursor = note.id === this.getSelectedNoteId() ? "›" : " ";
			rows.push(colors.highlight(`${cursor} ${index + 1}. ${note.title} · ${note.id}`));
