...
export function requestRuntimeRows(
	...
	const rows = section("Plan", width, "", a.plan);
	rows.push(...section("Stages", width, `${completed}/${request.stages.length}`, a.plan));
	...
	const activeStage = request.stages.find(stage => stage.status === "running")
		...
		?? request.stages.at(-1);
	if (activeStage?.goal) rows.push(...(compact
		? prose(a.caption(safe(activeStage.goal)), width, 1)
		: statusCardRows(activeStage.goal, activeStage.status, width)));
	for (const stage of request.stages.filter(stage => stage.skipReason)) {
		rows.push(...prose(a.muted(`${stage.id} · ${safe(stage.skipReason)}`), width, 2));
	}
	if (activityRows) rows.push(...activityRows);
	else {
		rows.push(...section("Activity", width, "", a.info));
		rows.push(...prose(a.muted("정리된 세부 작업이 도착하면 이곳에 표시합니다."), width));
	}
	const proposalRows = inputProposals.flatMap(proposal => compact
		? prose(a.caption(safe(proposal)), width, 1)
		: statusCardRows(proposal, "pending", width));
	rows.push(...section("Next", width, inputProposals.length ? `${inputProposals.length}개` : "없음", a.request), ...proposalRows);
	if (inputProposals.length === 0) rows.push(...prose(a.muted("다음 입력 제안이 없습니다."), width));
