...
export interface AstraDemoState {
...
function demoActivities(): ProjectActivity[] {
	...
	const activity = (id: string, sequence: number, kind: ProjectActivity["kind"], phase: ProjectActivity["phase"], payload: ProjectActivity["payload"]): ProjectActivity => ({
		schemaVersion: 1, id, projectId: "astra-demo", sequence, recordedAt: `2026-09-22T09:00:0${sequence}.000Z`, kind, phase,
	...
	return [
		...
		activity("demo-file", 5, "file-change", "completed", { method: "item/completed", params: { item: { type: "fileChange", path: "src/adapters/inbound/tui/features/demo/astra-demo.ts", changes: 3 } } }),
...
export function createAstraDemoState(_base: WorkbenchSnapshot, clock = Date.now): AstraDemoState {
