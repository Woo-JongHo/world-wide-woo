...
export class WwwDashboardView implements Component {
	...
	public render(width: number): string[] {
		...
		const summary = [
			...
			[compactSummary ? "Events" : "Progress events", number(snapshot.activities.length), live ? "working" : snapshotPhase(snapshot.phase)],
		...
		if (width < 58) {
			...
			return [
				...wwwSection("Session Overview", width, snapshotPhase(snapshot.phase), a.active),
				...summary.map(([title, value, detail]) => pair(title, `${value} · ${detail}`, width)),
				...wwwSection("Now", width, live ? "working" : "idle", live ? a.active : a.muted),
...
export class WwwDashboardRail implements Component {
	public constructor(
		private readonly getSnapshot: () => WorkbenchSnapshot,
		private readonly showSyntheticCatalog: () => boolean = () => false,
	) {}
