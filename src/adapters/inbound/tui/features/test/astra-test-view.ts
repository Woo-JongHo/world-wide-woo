...
import { projectRequestTestWorkspace, type RequestTestWorkspace } from "../../../../../core/domain/observability/request-test-workspace";
import type { WorkbenchSnapshot } from "../../../../../core/domain/work/workbench";
import { a, fit, mark, oneLine, prose, safe, section } from "../../foundation/theme/astra-theme";

const statusLabel = { running: "실행 중", passed: "통과", failed: "실패", blocked: "차단", skipped: "생략", planned: "계획", unknown: "미확인" } as const;
const kindLabel = { "black-box": "블랙박스 테스트", integration: "통합 테스트", regression: "회귀 테스트", unit: "단위 테스트", "static-analysis": "정적 검증", "read-back": "결과 재조회", manual: "수동 확인", unclassified: "분류 미기록" } as const;
...
export function projectAstraTestView(snapshot: WorkbenchSnapshot): RequestTestWorkspace {
	return projectRequestTestWorkspace({ requests: snapshot.requestRuntime, activities: snapshot.activities });
}

export function renderAstraTestView(workspace: RequestTestWorkspace, width: number): string[] {
	const rows = [...section("질문별 Test", width, `${workspace.groups.length}개 요청`), a.muted("무엇을 왜 확인했는지부터 읽습니다. 실행 횟수는 결과 뒤에 둡니다."), ""];
	if (!workspace.groups.length) rows.push(a.muted("이 세션에는 아직 질문이나 검증 기록이 없습니다."));
	for (const [index, group] of workspace.groups.entries()) {
		rows.push(`${mark(group.status)} ${a.strong(oneLine(group.question, Math.max(16, width - 8)))}`, a.muted(`  ${statusLabel[group.status]}  /  request ${safe(group.requestId)}`), a.note(`  ${group.rationale}`), "");
		...
		for (const kind of Object.keys(kindLabel) as Array<keyof typeof kindLabel>) {
			...
			for (const check of checks) {
				rows.push(`  └─ ${mark(check.status)} ${safe(check.title)}${check.evidenceActivityId ? `  ${a.muted(`/source ${safe(check.evidenceActivityId)}`)}` : ""}`, a.muted(`     └─ 목표 · ${safe(check.purpose)}`));
			}
		}
