...
/** Face highlight is constant; the mantle shell shades by light plane and scan sweep. */
function surfaceLight(point: Point, rotatedZ: number, rotatedX: number, scan: number): number {
	return point.face ? 0.95 : 0.43 + (rotatedZ + 0.8) * 0.2 - rotatedX * 0.10 - point.y * 0.08 + scan * 0.4;
}
