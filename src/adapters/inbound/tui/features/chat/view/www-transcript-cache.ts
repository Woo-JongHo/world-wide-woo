...
export interface WwwTranscriptCacheInput<DurableRevision, VolatileRevision> {
...
export type WwwTranscriptCacheMetrics = TranscriptCacheMetrics;
...
export class WwwTranscriptCache<DurableRevision, VolatileRevision> {
	...
	metrics(): WwwTranscriptCacheMetrics {
	...
	scrollRows(width: number, input: WwwTranscriptCacheInput<DurableRevision, VolatileRevision>): { readonly rowCount: number; readonly rows: (start: number, count: number) => string[] } {
	...
	private rowsForRange(
		...
		if (!Number.isSafeInteger(start)
			...
			throw new RangeError(`Invalid WWW transcript row range ${start}:${count}/${rowCount}`);
		}
	...
	private prepareGenerations(input: WwwTranscriptCacheInput<DurableRevision, VolatileRevision>): { durable: DurableTranscriptGeneration<DurableRevision>; volatile: VolatileTranscriptGeneration<VolatileRevision> } {
	...
	private newDurableGeneration(input: WwwTranscriptCacheInput<DurableRevision, VolatileRevision>, blocks: readonly TranscriptBlock[], widths: Map<number, TranscriptWidthIndex>): DurableTranscriptGeneration<DurableRevision> {
		const startedAt = performance.now();
	...
	private renderBlock(generation: TranscriptGeneration, blockIndex: number, width: number, retain: boolean, countBuild = false): readonly string[] {
		...
		if (!block) throw new RangeError(`Missing WWW transcript block ${blockIndex}`);
	...
	private rowsFrom(generation: TranscriptGeneration, index: TranscriptWidthIndex, start: number, count: number): string[] {
		...
		for (let blockIndex = low; blockIndex < generation.blocks.length && (index.prefix[blockIndex] ?? end) < end; blockIndex += 1) {
			...
			if (!block || expectedCount === undefined || blockStart === undefined) throw new RangeError(`Missing WWW transcript index ${blockIndex}`);
			if (blockRows.length !== expectedCount) throw new Error(`WWW transcript block ${block.key} changed row count at width ${index.width}`);
