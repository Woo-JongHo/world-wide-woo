...
import { conversationRecapRows }                                                from "@/adapters/inbound/tui/features/chat/conversation-recap-view";
import {
	a,
	astraMarkdownTheme,
	astraTitle,
...
import { CHAT_TERMINAL_OUTPUT_CHUNK_LINES }                                     from "@/adapters/inbound/tui/features/chat/chat-output-policy";
import { getActiveTuiTheme }                                                    from "@/adapters/inbound/tui/foundation/theme/theme";
import { AstraTranscriptCache }                                                 from "@/adapters/inbound/tui/features/chat/astra-transcript-cache";
import type {
	AstraTranscriptCacheInput,
	AstraTranscriptCacheMetrics,
	TranscriptBlock,
} from "@/adapters/inbound/tui/features/chat/astra-transcript-cache";

export type { AstraTranscriptCacheMetrics } from "@/adapters/inbound/tui/features/chat/astra-transcript-cache";
...
export function executionHeading(s: WorkbenchSnapshot): { state: string; title: string; detail: string; attention: boolean } {
	...
	if (request) {
		...
		return {
			...
			detail    : astraNowLabel(s) ?? "정리된 작업 내용을 기다리는 중",
	...
	if (s.phase === "working") {
		...
		return {
			...
			detail    : waiting ? "작업 진행 상태를 확인하는 중" : astraNowLabel(s) ?? "정리된 작업 내용을 기다리는 중",
			attention : waiting,
		};
	}
...
type ChatMessage = WorkbenchSnapshot["chat"][number];
type TNote = WorkbenchSnapshot["tnotes"][number];
...
function durableTimelineIndex(snapshot: WorkbenchSnapshot): DurableTimelineIndex {
...
export class AstraTranscriptView implements Component {
	...
	private readonly cache   = new AstraTranscriptCache<DurableTranscriptRevision, VolatileTranscriptRevision>() ;
	...
	private renderTNote(item: TNote, markdownKey: string, width: number): string[] {
		...
		if (!panels) {
			rows.push("", ...this.md(markdownKey, astraTNoteMarkdown(item), contentWidth).map(row => fit(`  ${row}`, width)), "");
	...
	private cacheInput(): AstraTranscriptCacheInput<DurableTranscriptRevision, VolatileTranscriptRevision> {
