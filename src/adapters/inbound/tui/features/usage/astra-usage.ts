...
function limitText(limit: UsageLimitSnapshot): string {
	...
	return (value >= 50 ? a.rainbowGreen : value >= 20 ? a.rainbowOrange : a.rainbowRed)(`${value}%`);
}

/** Account quota, never a dollar estimate. Full limits and reset times remain in /context. */
export function astraUsageLine(snapshots: readonly UsageSnapshot[], width: number, model = "", _now = Date.now(), showLogos = false): string {
	...
	const segment = ([id, name]: typeof providers[number], compact = false): string => {
		...
		const values = preferred && (s?.state === "ready" || s?.stale)
			? limitText(preferred)
	...
	const compact = providers.map(p => segment(p, true)).join("  ");
	if (visibleWidth(compact) <= width) return finish(compact);
	const activeProvider = providers.find(([id]) => id === active)!;
	return finish(`${segment(activeProvider, true)}  +${providers.length - 1}`);
}
