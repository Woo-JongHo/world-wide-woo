...
export function renderModelPickerView(state: ModelPickerViewState, width: number, ui: TuiColors): string[] {
	...
	if (state.appearance === "astra") return [
		...
		ui.muted(`현재 ${state.current.model} / ${state.current.effort}`), ui.muted(state.catalogNotice),
	...
	const result = [
		...
		fit(`현재: ${state.current.provider} / ${state.current.model} / ${state.current.effort}`, contentWidth),
		fit(ui.highlight(`선택: ${state.staged.provider} / ${state.staged.model} / ${state.staged.effort}`), contentWidth),
		fit(ui.border("─".repeat(contentWidth)), contentWidth),
	];
