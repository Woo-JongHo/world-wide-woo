...
export class WwwContextView implements Component {
	...
	render(width: number): string[] {
		...
		if (this.source) {
			...
			else if (s.selectedActivityId) rows.push(kv("선택한 Journal", s.selectedActivityId), a.attention("선택은 유지되지만 현재 관측 범위에 원본 기록이 없습니다."), a.muted("다른 기록으로 대체하지 않았습니다."));
		...
		const rows = [
			...
			pair(`Memory  ${number(memoryItems)} items`, `Chat ${s.chat.length}  ·  Journal ${s.activities.length}  ·  Note ${s.tnotes.length}`, width),
...
export class WwwContextRail implements Component {
	...
	render(width: number): string[] {
		...
		rows.push(metrics.used == null || metrics.total == null ? a.rule("━".repeat(Math.max(4, width - 2))) : wwwMeter(metrics.used, metrics.total, Math.max(4, width - 2), a.active));
