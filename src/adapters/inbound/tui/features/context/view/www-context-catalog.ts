...
import { a, wwwPalette, fit, pair, safe }            from "@/adapters/inbound/tui/foundation/theme/www-theme";
...
function sourceColor(color: typeof sources[number]["color"]): string {
	if (color === "text") return wwwPalette.cream;
	if (color === "failure" && getActiveTuiTheme() === "gruvbox") return "#af3a03";
	return wwwPalette[color];
}
...
function meter(value: number, total: number, width: number, ink: WwwInk, solid = false): string {
...
function dependencies(snapshot: WorkbenchSnapshot, width: number): string[] {
	...
	return [a.active("■ SYSTEM DEPENDENCY MAP"), a.active("                 WWW CORE"),
