...
export class NativeThreadPicker implements Component {
	...
	public constructor(
		...
		onCancel: () => void,
		private readonly surface?: "www",
	) {
		this.list = new SelectList(
			threads.map(thread => ({
				value       : thread.id,
				label       : surface ? safe(threadPreview(thread)) : threadPreview(thread),
				description : surface ? safe(threadDescription(thread)) : threadDescription(thread),
			})),
			Math.min(10, Math.max(1, threads.length)),
			surface ? wwwEditorTheme.selectList : selectListTheme,
	...
	public render(width: number): string[] {
		if (this.surface) return [a.strong("www") + a.muted(" / resume"), "", a.text("계속할 작업을 선택하세요"), a.muted("↑↓ 선택 / Enter 재개 / Esc 취소"), "", ...this.list.render(Math.max(1, width - 4))].map(row => fit("  " + row, width));
		return [
...
export function selectNativeThread(threads: readonly NativeThreadSummary[], surface?: "www"): Promise<string | null> {
	...
	return new Promise((resolve, reject) => {
		...
		const picker = new NativeThreadPicker(threads, id => finish(id), () => finish(null), surface);
		try {
