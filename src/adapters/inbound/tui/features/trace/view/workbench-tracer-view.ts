...
import type { TracerFeatureProjection }                    from "@/core/application/orchestration/workbench-feature-reads";
...
export interface TracerDelegationPresentation {
	readonly renderSummary: (tasks: readonly NativeDelegatedTask[], goal: string, width: number) => readonly string[];
	readonly renderDetail: (task: NativeDelegatedTask | null, width: number, activityLimit?: number) => readonly string[];
}
...
export class WorkbenchTracerView implements Component {
	public constructor(
		private readonly getSnapshot: () => TracerFeatureProjection,
		private readonly delegationPresentation: TracerDelegationPresentation | null = null,
	) {}
...
function delegationRows(snapshot: TracerFeatureProjection, width: number, presentation: TracerDelegationPresentation | null): string[] {
	if (!snapshot.delegation?.length || !presentation) return [];
	return [colors.border("─".repeat(width)),
		...presentation.renderSummary(snapshot.delegation.flatMap(entry => entry.tasks), snapshot.performance?.workContext?.goal ?? snapshot.performance?.request?.text ?? "", width),
		...presentation.renderDetail(snapshot.selectedAgentDetail ?? null, width, snapshot.delegationDetailActivities)];
}

function performanceHealthRows(snapshot: TracerFeatureProjection): string[] {
	const performance  = snapshot.performance                                                              ;
