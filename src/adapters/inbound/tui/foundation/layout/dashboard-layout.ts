/** @linear WOO-689 */
import { HStack, ScrollView, VStack, truncateToWidth, visibleWidth } from "@earendil-works/pi-tui";
import type { Component }                                            from "@earendil-works/pi-tui";
import { colors }                                                    from "@/adapters/inbound/tui/foundation/theme/theme";
import { DASHBOARD_PANEL_SYSTEM }                                    from "@/adapters/inbound/tui/foundation/layout/dashboard-panel-system";
...
function sectionWithoutTitle({ title: _title, titleMeta: _titleMeta, ...section }: DashboardSection): DashboardSection {
	return section;
}
...
class SectionDocument implements Component {
	...
	render(width: number): string[] {
		...
		if (!this.section.titleMeta
			&& width === this.cachedWidth
			&& childRows === this.cachedChildRows
			&& this.cachedRows) return this.cachedRows;
