...
// Astra 키맵 단일 원본. 바인딩을 키가 아니라 동작 이름(action)으로 소유하고,
// F키 테이블·Ctrl+G 뷰 테이블·HelpView·실행 문서의 키보드 표는 모두 여기서 파생한다.
// 셸 전역 리스너(workbench-shell)는 이 맵의 keyId만 소비하며, test/astra-keymap.test.ts가
// 맵·도움말·문서의 정합을 고정한다. 모달 오버레이 내부의 입력 키는 각 컴포넌트가 소유한다.

export type AstraKeyAction =
	| "page.execution"
	| "page.plan"
	| "page.monitor"
	| "page.stats"
...
export type AstraFunctionKey = "f2" | "f3" | "f4" | "f5" | "f6" | "f7" | "f8" | "f9";
export type AstraViewNumber = "1" | "2" | "3" | "4" | "5" | "6" | "7" | "8" | "9";

export interface AstraKeyBinding {
	readonly label        : string           ;
	readonly command?     : string           ;
	readonly functionKey? : AstraFunctionKey ;
	readonly viewNumber?  : AstraViewNumber  ;
...
export const ASTRA_VIEWS = Object.entries(ASTRA_KEYMAP)
	.filter((entry): entry is [AstraKeyAction, AstraKeyBinding & { command: string; viewNumber: AstraViewNumber }] =>
		entry[0] in ASTRA_KEYMAP && entry[1].viewNumber !== undefined && entry[1].command !== undefined)
...
export const ASTRA_KEYS = Object.entries(ASTRA_KEYMAP)
	.filter((entry): entry is [AstraKeyAction, AstraKeyBinding & { command: string; functionKey: AstraFunctionKey }] =>
		entry[0] in ASTRA_KEYMAP && entry[1].functionKey !== undefined && entry[1].command !== undefined)
...
export const ASTRA_HELP_ACTIONS = [
	"views.switcher",
	"command.palette",
	"browse.toggle",
	"scroll.move",
	...
	"interrupt.or.exit",
	"session.exit",
] as const satisfies readonly AstraKeyAction[];

/** 스크롤 이동의 별칭 키 그룹. 셸이 그대로 소비한다. */
export const ASTRA_SCROLL_KEYS = {
	down     : ["down", "j"],
	up       : ["up", "k"],
	pageDown : ["pageDown"],
	pageUp   : ["pageUp"],
...
export const ASTRA_DOC_EXTRA = ["F2–F9"] as const;

/** KeyId 유니온에 없는 시프트 문자(G)만 raw 비교하고, 나머지는 matchesKey가 Kitty 시퀀스까지 처리한다. */
export function matchesAstraKey(data: string, key: string): boolean {
	return key.length === 1 && key !== key.toLowerCase() ? data === key : matchesKey(data, key as KeyId);
}

export function matchesAstraAction(data: string, action: AstraKeyAction): boolean {
	return (ASTRA_KEYMAP[action].keys ?? []).some(key => matchesAstraKey(data, key));
}
