...
export const ASTRA_VIEWS = (Object.entries(ASTRA_KEYMAP) as [AstraKeyAction, AstraKeyBinding][])
	.filter(([, binding]) => binding.viewNumber)
	.sort(([a], [b]) => ASTRA_KEYMAP[a].viewNumber!.localeCompare(ASTRA_KEYMAP[b].viewNumber!))
	.map(([, binding]) => [binding.viewNumber!, binding.command!, binding.label] as const);

/** 보조 F키 테이블 — [키, 명령, 라벨]. 셸의 F키 루프가 소비한다. */
export const ASTRA_KEYS = (Object.entries(ASTRA_KEYMAP) as [AstraKeyAction, AstraKeyBinding][])
	.filter(([, binding]) => binding.functionKey)
	.sort(([a], [b]) => ASTRA_KEYMAP[a].functionKey!.localeCompare(ASTRA_KEYMAP[b].functionKey!))
