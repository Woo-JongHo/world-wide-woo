...
export class PiHarness implements ExecutorPort {
	...
	public async readThread(input: NativeThreadRead): Promise<NativeThreadSnapshot> {
		if (this.closed
			...
			|| !this.session) throw new Error("Pi thread is not loaded");
		return {
			id: this.threadId,
			value: {
	...
	public async interruptTurn(input: NativeTurnInterrupt): Promise<void> {
		if (!this.active
			|| input.threadId !== this.threadId
			|| input.turnId !== this.active.turnId
			|| !this.session) throw new Error("Pi turn is not active");
