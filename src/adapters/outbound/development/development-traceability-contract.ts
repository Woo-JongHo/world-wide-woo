...
const digestWithout          = (value: object, field = "payloadDigest") => { const { [field]: _, ...body } = value as Record<string, unknown>; return canonicalDigest(body); } ;
...
export function requiredCoverageFromRegistries(
	...
	for (const entity of ledger.entities.filter((value) =>
		...
		if (envelopeErrors.length
			...
			|| envelope.id !== entity.id
			|| envelope.version !== entity.version) {
...
export function validateRegistryEnvelope(envelope: RegistryEnvelope<unknown>): string[] {
	...
	if (envelope.payloadDigest !== digestWithout(envelope)) errors.push("REGISTRY_DIGEST_MISMATCH");
	const payload = envelope.payload as Partial<SpecContract & TestContract & ExceptionContract>;
	if (envelope.kind === "spec") for (const acceptance of (payload.acceptances ?? [])) if (!acceptance.id
		|| !acceptance.purpose
		|| !acceptance.risk
		|| !acceptance.pass) errors.push("ACCEPTANCE_PURPOSE_RISK_PASS_REQUIRED");
...
export function validateVerificationReceipt(receipt: VerificationReceipt, expectedSourceRevision?: string): string[] {
	...
	if (receipt.payloadDigest !== digestWithout(receipt)) errors.push("RECEIPT_DIGEST_MISMATCH");
	for (const evidence of receipt.evidence) if (!/^EV-[A-Za-z0-9-]+$/u.test(evidence.id)
		|| !safePath(evidence.path)
		|| !SHA256.test(evidence.sha256)
		|| !evidence.immutable) errors.push("EVIDENCE_DIGEST_MISMATCH");
...
export function verificationReceiptFromCompletion(completion: CompletionReceipt, context: VerificationReceiptCompletionContext): VerificationReceipt {
	...
	if (
		!completion.runId ||
		!SHA256.test(completion.checkpointDigest) ||
		!context.observedSourceRevision ||
		!context.execution?.environment ||
		!Array.isArray(context.evidenceArtifacts) ||
		!context.evidenceArtifacts.length ||
		!Array.isArray(context.acceptanceCoverage) ||
		!Array.isArray(context.requiredAcceptanceRefs) ||
		!Array.isArray(context.exceptionStageCoverage) ||
		!Array.isArray(context.applicableExceptionStages) ||
		!Array.isArray(context.limitations) ||
		!context.limitations.length
	) {
		throw new Error("COMPLETION_CONTEXT_REVISION_EVIDENCE_EXECUTION_COVERAGE_LIMITATIONS_REQUIRED");
	}
	...
	const receipt: Required<VerificationReceipt> = {
	...
	receipt.payloadDigest = digestWithout(receipt);
...
export function validateTraceabilityLedgerV3(ledger: TraceabilityLedgerV3, computedPayloadDigest = canonicalDigest(omitDigest(ledger))): string[] {
	...
	for (const entity of ledger.entities) {
		const match = REF.exec(entity.ref); if (!match
			|| match[1] !== entity.kind
			|| !entity.id
			|| entity.ref !== `${entity.kind}:${entity.id}${entity.version ? `@v${entity.version}` : ""}`) errors.push(`ENTITY_REF_INVALID:${entity.ref}`);
	...
	const byRef = new Map(ledger.entities.map(entity => [entity.ref, entity]));
	for (const edge of ledger.edges) {
		const from = byRef.get(edge.from);
		const to = byRef.get(edge.to);
		if (!from || !to) { errors.push(`DANGLING_EDGE:${edge.from}:${edge.to}`); continue; }
