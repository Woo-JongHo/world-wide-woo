...
import type {
	...
	DevelopmentTest,
	DevelopmentUnit,
	RecordDevelopmentTestInput,
} from "@/core/domain/development/development-records.js";
import type {
	TraceabilityEdge,
	TraceabilityLedger,
	TraceRef,
...
import { parseWorkTraceabilityManifest, referenceKey } from "@/core/domain/work/traceability.js";
import type {
	WorkReference,
	WorkReferenceKind,
	WorkTraceabilityLink,
	WorkTraceabilityManifest,
} from "@/core/domain/work/traceability.js";
import { digestLedger, stableJson }                    from "@/adapters/outbound/development/development-traceability-digest.js";
import {
	canonicalDigest,
...
import {
	inspectObsidianTraceability,
	validateReceiptEvidenceAlignment,
} from "@/adapters/outbound/development/traceability-validator.js";
import type { ObsidianNoteProjection }                 from "@/adapters/outbound/development/traceability-validator.js";
...
type Entity = DevelopmentUnit | DevelopmentBinding | DevelopmentRecord | DevelopmentTest | LinkEntity;
...
export class DevelopmentStore {
 ...
 private readonly vaultSpecRoot?          : string | undefined            ;
 private readonly requiredVaultLinearIds? : readonly string[] | undefined ;
 ...
 private scan(): { sources: Source[]; errors: string[] } {
  ...
  for (const root of [this.metadataRoot, this.sourceRoot]) for (const name of readdirSync(root).sort()) {
   ...
   try {
	...
	|| !["unit", "link", "binding", "record", "test"].includes(envelope.kind)
	|| !envelope.payload
	|| typeof envelope.payload.id !== "string"
	|| envelope.digest !== hash(JSON.stringify(envelope.payload))) throw new Error("Invalid envelope or source digest");
  ...
  const links                           = this.entities<LinkEntity>(sources, "link")            ;
  ...
  for (const source of sources) try {
   ...
   if (kind === "link") {
    const link = payload as LinkEntity;
   ...
   if (["binding", "record", "test"].includes(kind)) {
	...
	|| !value.issueIds.length
	|| value.unitIds.some(id => !units.some(unit => unit.id === id))
	|| value.issueIds.some(id => !issueIds.includes(id))) throw new Error("Invalid or dangling attribution");
    if (kind === "binding") {
     const binding = value as DevelopmentBinding;
     const versionKey = `binding-version:${binding.runId}:${binding.version}`;
     ...
     else { const test = record as DevelopmentTest; nonempty(test.command, "Test command"); nonempty(test.cwd, "Test cwd"); if (typeof test.output !== "string"
	|| !["passed", "failed", "cancelled", "not-run"].includes(test.status)
	|| (test.status === "passed" && test.exitCode !== 0)
	|| (test.status === "failed" && (test.exitCode === null || test.exitCode === 0))) throw new Error("Invalid test result"); }
    }
   }
 ...
 private expectedTraceability(ledger: TraceabilityLedger) {
  ...
  if (this.vaultRoot) {
   const inspected = inspectObsidianTraceability({
    vaultRoot: this.vaultRoot,
    ledger,
    ...(this.vaultSpecRoot ? { specRoot: this.vaultSpecRoot } : {}),
    ...(this.requiredVaultLinearIds ? { requiredLinearIds: this.requiredVaultLinearIds } : {}),
   });
  ...
  const acceptanceCoverage = ledger.entities.filter(entity => entity.kind === "acceptance").map(entity => {
   ...
   const candidates = tests.flatMap(test => currentReceipts(entity.ref, test).flatMap(receiptRef => {
    const receipt = receipts.get(receiptRef);
    if (!receipt) return [];
    return [{ test, receiptRef, coverage: receipt.acceptanceCoverage.find(item => item.acceptanceRef === entity.ref && item.testRef === test) }];
   }));
   const candidate = candidates.length === 1 ? candidates[0] : undefined;
   return { acceptanceRef: entity.ref, testRef: candidate?.test ?? (tests.length === 1 ? tests[0] ?? null : null), receiptRef: candidate?.receiptRef ?? null, status: candidate?.coverage?.status ?? "blocked" };
  ...
  const exceptionCoverage = ledger.entities.filter(entity => entity.kind === "exception").flatMap(entity => ["detect", "control", "recovery"].map(stage => {
   ...
   const candidates = tests.flatMap(test => currentReceipts(entity.ref, test).flatMap(receiptRef => {
    const receipt = receipts.get(receiptRef);
    if (!receipt) return [];
    return [{ test, receiptRef, coverage: receipt.exceptionCoverage.find(item => item.exceptionRef === entity.ref && item.stage === stage && item.testRef === test) }];
   })).filter(candidate => candidate.coverage);
   const candidate = candidates.length === 1 ? candidates[0] : undefined;
   return { exceptionRef: entity.ref, stage, testRef: candidate?.test ?? (tests.length === 1 ? tests[0] ?? null : null), receiptRef: candidate?.receiptRef ?? null, status: candidate?.coverage?.status ?? "blocked" };
 ...
 assertTraceabilityCurrent(): { logicalDigest: string; rowDigest: string } {
  ...
  if (stableJson(acceptanceCoverage) !== stableJson(expected.acceptanceCoverage)
	|| stableJson(exceptionCoverage) !== stableJson(expected.exceptionCoverage)
	|| stableJson(freshness) !== stableJson(expected.freshness)
	|| stableJson(notes) !== stableJson(expected.notes)) errors.push("traceability materialized projection differs from relation ledger");
 ...
 bindRun(input: { runId: string; issueIds: string[]; unitIds: string[] }): DevelopmentBinding {
  ...
  return this.transaction(sources => {
   const units = this.entities<DevelopmentUnit>(sources, "unit");
   const issues = [...this.entities<LinkEntity>(sources, "link").map(x => x.issue.id), ...this.legacy().references.filter(x => x.kind === "linear-issue").map(x => x.id)];
