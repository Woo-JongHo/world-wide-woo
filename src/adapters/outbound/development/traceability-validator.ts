...
import { digestLedger }                           from "@/adapters/outbound/development/development-traceability-digest.js";
import {
	canonicalDigest,
...
import type { ObsidianVaultDocument }             from "@/adapters/outbound/development/obsidian-contract.js";
...
function wikiAliases(document: ObsidianVaultDocument): string[] {
	...
	return filename ? [withoutExtension, filename] : [withoutExtension];
}

function isLinearSnapshotIssue(value: unknown): value is { id: string; description?: string } {
...
export function inspectObsidianTraceability(options: {
	...
	const inspected = inspectObsidianVault(options.vaultRoot, {
		...(options.specRoot ? { specRoot: options.specRoot } : {}),
		...(options.requiredLinearIds ? { requiredLinearIds: options.requiredLinearIds } : {}),
	});
	...
	for (const document of documents) {
		...
		for (const relationship of relationships) {
			...
			const targetRef = `note:${targetDocument.documentId}`   ;
	...
	if (options.linearSnapshot) {
		const issues = options.linearSnapshot.filter(isLinearSnapshotIssue);
		...
		for (const issue of issues.filter(candidate => targetIssues.has(candidate.id))) {
			...
			if (detailEdges.length !== 1) errors.push(`LINEAR_DETAILED_BY_EXACTLY_ONE_REQUIRED:${issue.id}:${detailEdges.length}`);
				...
				const detailEdge = detailEdges[0];
				if (detailEdge && !notes.some(note => `note:${note.documentId}` === detailEdge.to)) {
					errors.push(`LINEAR_DETAILED_BY_CANONICAL_TARGET_REQUIRED:${issue.id}:${detailEdge.to}`);
				}
			}
		}
	}
...
export function validateReceiptEvidenceAlignment(ledger: TraceabilityLedger, receipts: ReadonlyMap<string, VerificationReceipt>): string[] {
	...
	for (const entity of evidenceEntities) {
		...
		if (!entity.immutable
			|| !entity.source
			|| !expected
			|| entity.source.path !== expected.path
			|| entity.source.digest !== expected.digest) {
...
export async function validateTraceability(options: { projectRoot: string; ledger: TraceabilityLedger; vaultRoot: string; specRoot?: string; linearSnapshot?: readonly unknown[]; requiredLinearIds?: readonly string[] }): Promise<string[]> {
	...
	const noteInspection = inspectObsidianTraceability({
		vaultRoot: options.vaultRoot,
		ledger,
		...(options.specRoot ? { specRoot: options.specRoot } : {}),
		...(options.linearSnapshot ? { linearSnapshot: options.linearSnapshot } : {}),
		...(options.requiredLinearIds ? { requiredLinearIds: options.requiredLinearIds } : {}),
	});
