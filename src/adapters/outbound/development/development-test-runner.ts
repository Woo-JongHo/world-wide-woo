...
import { captureDevelopmentSnapshot } from '@/adapters/outbound/development/development-snapshot.js';
import type { DevelopmentSnapshot }   from '@/adapters/outbound/development/development-snapshot.js';
...
export interface DevelopmentTestExecution {
  ...
  startedAt    : string                                                      ;
	finishedAt : string                                                      ;
	snapshot   : { before: DevelopmentSnapshot; after: DevelopmentSnapshot } ;
}
...
export async function runDevelopmentTest(input: { argv: string[]; projectRoot: string; artifactRoot: string; signal?: AbortSignal; bindingId?: string }): Promise<DevelopmentTestExecution> {
  ...
  const cwd                                         = resolve(input.projectRoot)                          ;
  const before                                      = captureDevelopmentSnapshot(cwd, input.artifactRoot) ;
