import type { LinearDashboardComment, LinearDashboardIssue, LinearDashboardMilestone, LinearDashboardUpdate, LinearProjectDashboard } from "../../../core/domain/work/linear-dashboard.js";
...
export class McpLinearProjectDashboard {
	...
	public async refresh(threadId: string): Promise<LinearProjectDashboard> {
		const [issues, updates, milestones, comments] = await Promise.all([
			...
			this.call("list_comments", { projectId: this.config.projectId, limit: 5, orderBy: "createdAt" }, threadId).catch(() => ({ comments: [] })),
		]);
		return { state: "ready", projectName: this.config.projectName, fetchedAt: new Date().toISOString(), issues: projectIssues(issues), update: projectUpdate(updates), comments: projectComments(comments), milestones: projectMilestones(milestones), error: null };
	}
...
function projectComments(value: unknown): readonly LinearDashboardComment[] {
	return list(value, "comments").map(comment => {
