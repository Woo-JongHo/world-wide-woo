...
export function validateLedger(ledger: TraceabilityLedger, computedPayloadDigest: string): string[] {
	...
	for (const issue of ledger.issues) {
		...
		try {
			...
			validUrl = url.protocol === "https:" && url.hostname === "linear.app" && new RegExp(`/issue/${issue.id}(?:/|$)`, "u").test(url.pathname);
		} catch { /* Report the common identity error below. */ }
		if (!/^WOO-\d+$/u.test(issue.id) || !UUID.test(issue.uuid) || !validUrl) errors.push(`invalid Linear identity: ${issue.id}`);
	}
