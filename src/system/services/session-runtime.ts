...
import type { WwwSettings } from "../contracts/model-settings";
import { isPublicNarrationText, workNarrationLabel, workNarrationReason, type WorkNarration } from "../contracts/narration";
import type { PlanningSnapshot } from "../contracts/planning";
import type { SessionEvent } from "../contracts/session-events";
import type { CommandResultSnapshot, GenericToolResultSnapshot, ToolResultSnapshot } from "../contracts/output";
import { sanitizeTerminalText } from "../contracts/terminal";
import type { AgentTool, ModelAuthStatus, ModelClient, SessionRepository, TerminalCommandExecutor, TodoController } from "../contracts/ports";
...
export class SessionRuntime {
	...
	runTerminalCommand(command: string): Promise<void> {
		...
		if (!this.terminal) return Promise.reject(new Error("이 WWW 실행에는 terminal command executor가 연결되지 않았습니다."));
		...
		const task = this.runDirectTerminalCommand(rawCommand);
		this.activeTask = task;
		void task.then(
			() => { if (this.activeTask === task) this.activeTask = null; },
			() => { if (this.activeTask === task) this.activeTask = null; },
	...
	private async runDirectTerminalCommand(rawCommand: string): Promise<void> {
		...
		try {
			...
			try {
				...
				result = {
					stdout: "",
					stderr: sanitizeTerminalText(errorMessage(error), 32 * 1_024),
					exitCode: null,
			...
			if (passed) await this.recordTodoEvidence(completedEvent.id, turnId, itemId);
	...
	private async recordTodoEvidence(evidenceId: string, turnId: string, itemId: string): Promise<void> {
		if (!this.todos) return;
		try {
			await this.todos.recordEvidence(evidenceId);
		} catch (error) {
			await this.store.append(this.id, {
				category: "warning",
	...
	private restore(events: readonly SessionEvent[]): void {
		...
		for (const event of events) {
			...
			if (event.type === "message.user") {
				...
				if (event.metadata.source !== "terminal") {
