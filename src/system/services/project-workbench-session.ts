...
import { ProjectWorkbench, type ProjectWorkbenchOptions, type WorkbenchActivityJournal, type WorkbenchTNoteSource, type WorkbenchTodoSource } from "./project-workbench.js";
import type { ExecutionLane, ExecutorPort, NativeHarnessSelection } from "../contracts/ports/executor-port.js";
import type { ComposerDraftController, DevelopmentSession, SessionRepository, TodoStore, UsageMonitor } from "../contracts/ports/index.js";
import { TNoteService } from "./t-note-service.js";
import type { ActivityNarrator } from "./activity-narrator.js";
import { WooEntry } from "./woo-entry.js";
import { SessionModelUsageAccumulator, type SessionModelUsageObservation } from "./session-model-usage.js";
import { TodoLedger } from "./todo-ledger.js";
import type { TodoDocument, TodoNativePlanBinding } from "../contracts/todos.js";
import type { TNoteDraft } from "../contracts/t-notes.js";
import type { WorkbenchModelSelection } from "../contracts/workbench.js";
import type { WorkFlowProjection } from "../contracts/work/index.js";
import type { ProjectActivity } from "../contracts/project-activity.js";
import { CanonicalPromotionService } from "./canonical-promotion.js";
import { ReviewService } from "./review-service.js";
import type { ProjectWorkspace, SessionLease } from "../contracts/project-workspace.js";

const WORKBENCH_RUN_PREFIX = "workbench";
export const DEFAULT_TNOTE_MODEL = "gpt-5.6-luna";

function digestActivitySource(source: string): string {
	return `sha256:${createHash("sha256").update(source).digest("hex")}`;
}

function nativeThreadJournalKey(threadId: string): string {
	if (!threadId.trim()) throw new Error("Native thread id is required for activity journal");
...
export interface ProjectWorkbenchSessionFactories {
	...
	createDevelopment?(root: string, runId: string): DevelopmentSession;
}

let productionFactories: ProjectWorkbenchSessionFactories | undefined;

/** The composition root installs production adapters; tests may override individual seams. */
export function configureProjectWorkbenchSessionFactories(factories: ProjectWorkbenchSessionFactories): void {
	productionFactories = factories;
}
...
export async function createProjectWorkbenchSession(
	...
	if (!productionFactories) throw new Error("Project Workbench production factories are not configured");
