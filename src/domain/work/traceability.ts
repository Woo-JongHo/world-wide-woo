...
export type WorkReference =
	| CanonicalReference<"epic" | "story" | "project-activity" | "native" | "code" | "test" | "evidence">
	| LinearIssueReference;

export type WorkTraceabilityRelation = "implements" | "verifies" | "evidences" | "tracks" | "originated-from";

export interface WorkTraceabilityLink {
	readonly from: WorkReference;
	readonly relation: WorkTraceabilityRelation;
...
const relationEndpoints: Readonly<Record<
	WorkTraceabilityRelation,
	readonly [from: readonly WorkReferenceKind[], to: readonly WorkReferenceKind[]]
>> = {
	implements: [["epic", "story", "linear-issue"], ["code"]],
	tracks: [["epic", "story"], ["linear-issue"]],
...
export function parseWorkTraceabilityManifest(value: unknown): WorkTraceabilityManifest {
	if (!isRecord(value) || value.schemaVersion !== 1 || !Array.isArray(value.references) || !Array.isArray(value.links)) {
		throw new Error("작업 추적 manifest가 올바르지 않습니다");
	}
	const references = value.references.map(parseReference);
	const known = new Map<string, WorkReference>();
	const linearUuidOwners = new Map<string, string>();
	const linearUrlOwners = new Map<string, string>();
	for (const reference of references) {
		const key = referenceKey(reference);
		if (known.has(key)) throw new Error(`작업 참조가 중복되었습니다: ${key}`);
		known.set(key, reference);
		if (reference.kind === "linear-issue") {
			assertUniqueLinearIdentity(linearUuidOwners, reference.uuid.toLowerCase(), key, "UUID");
			assertUniqueLinearIdentity(linearUrlOwners, reference.url, key, "URL");
		}
	}
	...
	for (const link of links) {
		for (const endpoint of [link.from, link.to]) {
			...
			if (!declared || JSON.stringify(declared) !== JSON.stringify(endpoint)) {
				throw new Error(`선언되지 않은 작업 참조입니다: ${referenceKey(endpoint)}`);
			}
		}
		const identity = `${referenceKey(link.from)}|${link.relation}|${referenceKey(link.to)}`;
		if (identities.has(identity)) throw new Error(`작업 추적 연결이 중복되었습니다: ${identity}`);
		identities.add(identity);
	}
	return Object.freeze({ schemaVersion: 1, references: Object.freeze(references), links: Object.freeze(links) });
}

export function relatedWorkLinks(manifest: WorkTraceabilityManifest, reference: WorkReference): readonly WorkTraceabilityLink[] {
	assertReference(reference);
	const key = referenceKey(reference);
	return Object.freeze(manifest.links.filter(link => referenceKey(link.from) === key || referenceKey(link.to) === key));
}
...
export function findWorkReference(manifest: WorkTraceabilityManifest, query: string): WorkReference | undefined {
	...
	const matches = manifest.references.filter(reference => {
		...
		return reference.kind === "linear-issue" && (reference.uuid.toLowerCase() === normalized.toLowerCase() || reference.url === normalized);
	});
	if (matches.length > 1) throw new Error(`작업 참조 조회 결과가 모호합니다: ${normalized}`);
	return matches[0];
}
...
function parseLink(value: unknown): WorkTraceabilityLink {
	if (!isRecord(value)) throw new Error("작업 추적 연결이 올바르지 않습니다");
	const relation = value.relation;
	if (!["implements", "verifies", "evidences", "tracks", "originated-from"].includes(String(relation))) {
		throw new Error(`작업 추적 관계가 올바르지 않습니다: ${String(relation)}`);
	}
	const typedRelation = relation as WorkTraceabilityRelation;
	...
	if (!allowedFrom.includes(from.kind) || !allowedTo.includes(to.kind)) {
		throw new Error(`${typedRelation} 관계 방향이 올바르지 않습니다: ${from.kind} -> ${to.kind}`);
	}
	return Object.freeze({ from, relation: typedRelation, to });
}

function parseReference(value: unknown): WorkReference {
	if (!isRecord(value) || typeof value.kind !== "string" || typeof value.id !== "string") throw new Error("작업 참조가 올바르지 않습니다");
	...
	if (typeof value.uuid !== "string" || !uuidPattern.test(value.uuid)) throw new Error(`Linear UUID가 올바르지 않습니다: ${String(value.uuid)}`);
	if (typeof value.url !== "string" || !isLinearIssueUrl(value.url, value.id)) throw new Error(`Linear 이슈 URL이 올바르지 않습니다: ${String(value.url)}`);
...
function assertReference(reference: WorkReference): void {
	...
	if (!pattern || !pattern.test(reference.id)) throw new Error(`${reference.kind} 참조가 올바르지 않습니다: ${reference.id}`);
	if (isRepositoryPathReference(reference) && reference.id.split("/").includes("..")) {
		throw new Error(`${reference.kind} 참조가 올바르지 않습니다: ${reference.id}`);
	}
	if (reference.kind === "linear-issue" && (!uuidPattern.test(reference.uuid) || !isLinearIssueUrl(reference.url, reference.id))) {
		throw new Error(`Linear 이슈 참조가 올바르지 않습니다: ${reference.id}`);
	}
}

function assertUniqueLinearIdentity(owners: Map<string, string>, identity: string, owner: string, label: string): void {
	const existing = owners.get(identity);
	if (existing && existing !== owner) throw new Error(`Linear ${label}가 중복되었습니다: ${identity}`);
