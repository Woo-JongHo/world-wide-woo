...
export class LocalWorkflowService {
 ...
 async resume(runId: string): Promise<LocalWorkflowResult> {
  ...
  const subjectDigest = state.subjectDigest;
  ...
  const check: LocalWorkflowCheck = currentRegistry.digest !== state.scenario.registryDigest
   ? {status: "failed", subjectDigest, checkedAt: new Date().toISOString(), evidence: ["Skill 레지스트리 revision 변경"], issues: ["레지스트리 스냅샷이 변경됐습니다(저장소 HEAD 포함). Skill 파일 변경 여부와 별개로 새 Run으로 다시 검사하세요."]}
   : await this.verify({subjectDigest});
  const evidence = [...check.evidence, ...check.issues];
  if (!evidence.length) evidence.push("로컬 검사 결과 확인");
  let verification: SkillVerification | undefined = undefined;
  if (check.status === "passed") {
   const activeStep = state.activeIndex === null ? undefined : state.steps[state.activeIndex];
   if (!activeStep) throw new Error("SKILL_RUN_ACTIVE_STEP_INVALID");
   verification = {
    validator: "local-unit-references@1", scope: "local-preflight", runId, skill: activeStep.skill,
