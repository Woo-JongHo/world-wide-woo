...
import type { RpaScenario }       from "@/core/agents/rpa-agent.js";
...
export function startSkillRun(scenario: RpaScenario, runId: string = randomUUID(), scope: "full" | "local-preflight" = "full"): SkillRunState {
	...
	const steps: readonly SkillStepState[] = scenario.skillNames.map(skill => ({ skill, status: "pending", candidateId: null, candidateDigest: null, authorizationDigest: null, evidence: [] }));
...
export function authorizeSkillStep(state: SkillRunState, candidateDigest: string, actor: string): SkillRunState {
	...
	if (!active) throw new Error("SKILL_RUN_STATE_INVALID");
...
export function finishSkillStep(state: SkillRunState, status: WooReceiptStatus, evidence: readonly string[], verification?: SkillVerification): { state: SkillRunState; receipt: SkillRunReceipt } {
  ...
  if (!verification || state.scope !== "local-preflight" || verification.scope !== state.scope
   ...
   || JSON.stringify(verification.evidence) !== JSON.stringify(evidence)) throw new Error("SKILL_VERIFICATION_REQUIRED");
	...
	const unsigned: Omit<SkillRunReceipt, "receiptDigest"> = {
		schemaVersion : "1.0",
		receiptId     : randomUUID(),
		runId         : state.runId,
		skill         : { name: active.skill, version: "1.0" },
		capability    : active.skill.toUpperCase().replaceAll("-", "_") + (state.scope === "local-preflight" ? "_LOCAL_PREFLIGHT" : ""),
		status,
		stage            : "verify",
		candidateId      : active.candidateId,
		actor            : { type: "skill-runtime" },
		context          : { scope: state.scope ?? "full", intent: state.scenario.intent, processId: state.scenario.processId, taskId: state.scenario.taskId, unitIds: state.scenario.unitIds, registryDigest: state.scenario.registryDigest },
		input            : { candidateDigest: active.candidateDigest },
		decision         : { terminal },
		validation       : verification ? [{ ...verification }] : [{ status, evidence, verified: false }],
		authorization    : active.authorizationDigest ? { digest: active.authorizationDigest } : null,
		execution        : { revision: state.revision },
		result           : status === "succeeded" ? { step: active.skill, scope: state.scope, fullReconciliation: "not_verified" } : null,
		failure          : status === "succeeded" ? null : { status },
		evidence         : [...active.evidence, ...evidence],
		nextCapabilities : nextSkill ? [nextSkill] : [],
	};
	return { state: nextState, receipt: Object.freeze({ ...unsigned, receiptDigest: digest(unsigned) }) };
}

export function verifySkillRunReceipt(receipt: SkillRunReceipt & { readonly _storeRootDigest?: string }): void {
	const { receiptDigest, _storeRootDigest: _metadata, ...unsigned } = receipt;
...
export function skillRunMonitor(state: SkillRunState) {
	const active = state.activeIndex === null ? null : state.steps[state.activeIndex] ?? null;
