...
import { OBSIDIAN_SECTIONS }      from "@/core/domain/development/obsidian-contract.js";
import {
	renderRpaProject,
	renderRpaTask,
	validateRpaDescriptionMap,
} from "@/core/domain/development/rpa-description.js";
import type { RpaDescriptionMap } from "@/core/domain/development/rpa-description.js";
...
type ProjectActivityType  = typeof PROJECT_ACTIVITY_TYPES[number]  ;
type ProjectActivityState = typeof PROJECT_ACTIVITY_STATES[number] ;
type ProjectUpdateHealth  = typeof PROJECT_UPDATE_HEALTH[number]   ;
...
const isProjectActivityType  = (value: unknown): value is ProjectActivityType  => (PROJECT_ACTIVITY_TYPES  as readonly unknown[]).includes(value)                          ;
const isProjectActivityState = (value: unknown): value is ProjectActivityState => (PROJECT_ACTIVITY_STATES as readonly unknown[]).includes(value)                          ;
const isProjectUpdateHealth  = (value: unknown): value is ProjectUpdateHealth  => (PROJECT_UPDATE_HEALTH  as readonly unknown[]).includes(value)                           ;
...
function validateProjectCommentCandidate(candidate: ArtifactCandidate, content: Record<string, unknown>, errors: string[]): void {
	...
	if (legacy) {
		if (!line(content.requirement)
			|| !lines(content.work)
			|| !lines(content.connections)
			|| !line(content.next)) errors.push("linear-project-comment.content: requirement/work/connections/next가 필요합니다.");
		if (!Array.isArray(content.types) || !content.types.length || content.types.some(type => !isProjectActivityType(type))) errors.push("linear-project-comment.content.types: feature|improvement|refactor|fix|verification|operations 중 하나 이상이어야 합니다.");
		if (!isProjectActivityState(content.state)) errors.push("linear-project-comment.content.state: complete|in-progress|blocked여야 합니다.");
		return;
	}
	if (!lines(content.changes)
		|| !lines(content.impacts)
		|| !lines(content.verification)
		|| !lines(content.connections)) errors.push("linear-project-comment.content: changes/impacts/verification/connections가 필요합니다.");
	if (!Array.isArray(content.categories) || !content.categories.length || content.categories.some(type => !isProjectActivityType(type))) errors.push("linear-project-comment.content.categories: feature|improvement|refactor|fix|verification|operations 중 하나 이상이어야 합니다.");
}
...
export function validateArtifactCandidate(candidate: ArtifactCandidate, actualBefore?: unknown): string[] {
	...
	if (candidate.kind === "linear-issue" && !rpaTask) {
		const baseKeys    = ["title", "purpose", "included", "excluded", "done", "connections"] ;
		const allowedKeys = new Set([...baseKeys, "result", "behavior"])                        ;
		const detailed    = "result" in content || "behavior" in content                        ;
		if (Object.keys(content).some(key => !allowedKeys.has(key))) errors.push("linear-issue.content: 정해진 필드만 허용합니다.");
		if (!line(content.title)
			|| !line(content.purpose)
			|| !lines(content.included)
			|| !lines(content.excluded)
			|| !lines(content.done)
			|| !lines(content.connections)) errors.push("linear-issue.content: title/purpose와 included/excluded/done/connections가 필요합니다.");
		if (detailed && (!line(content.result) || !lines(content.behavior))) errors.push("linear-issue.content: result와 behavior는 함께 지정해야 합니다.");
	}
	...
	if (candidate.kind === "linear-project-update") {
		...
		if (!/^0\.0\.\d+$/u.test(String(content.version ?? ""))
			|| !lines(content.delivered)
			|| !lines(content.included)
			|| !lines(content.verification)
			|| !lines(content.connections)
			|| !lines(content.sourceCommentIds)) errors.push("linear-project-update.content: version과 delivered/included/verification/connections/sourceCommentIds가 필요합니다.");
		if (!isProjectUpdateHealth(content.health)) errors.push("linear-project-update.content.health: onTrack|atRisk|offTrack여야 합니다.");
	}
...
export function renderArtifactCandidate(candidate: ArtifactCandidate): string {
	...
	if (candidate.kind === "linear-issue") {
		...
		return `# ${c.title}\n\n## 목적\n\n${c.purpose}${result}\n\n## 범위\n\n### 포함\n\n${bullets(c.included)}\n\n### 제외\n\n${bullets(c.excluded)}${behavior}\n\n## 완료 조건\n\n${bullets(c.done)}\n\n## 연결\n\n${bullets(c.connections)}\n`;
	}
	if (candidate.kind === "linear-project-comment") {
		if (candidate.schemaVersion === "1.0") return `**요구**\n${bullets([c.requirement])}\n\n**작업**\n${bullets(c.work)}\n\n**유형**\n${bullets((c.types as string[]).map(type => PROJECT_ACTIVITY_TYPE_LABELS[type] ?? type))}\n\n**연결**\n${bullets(c.connections)}\n\n**상태**\n- ${PROJECT_ACTIVITY_STATE_LABELS[String(c.state)] ?? String(c.state)} — ${c.next}\n`;
		return `## 변경\n\n${bullets(c.changes)}\n\n## 영향\n\n${bullets(c.impacts)}\n\n## 분류\n\n${(c.categories as string[]).map(type => PROJECT_ACTIVITY_CATEGORY_LABELS[type] ?? type).join(" · ")}\n\n## 검증\n\n${bullets(c.verification)}\n\n## 연결\n\n${bullets(c.connections)}\n`;
	}
...
const PROJECT_ACTIVITY_TYPE_LABELS     : Record<string, string> = { feature: "기능", improvement: "개선", refactor: "리팩터링", fix: "수정", verification: "검증", operations: "운영" }                     ;
const PROJECT_ACTIVITY_CATEGORY_LABELS : Record<string, string> = { feature: "Feature", improvement: "Improvement", refactor: "Refactor", fix: "Fix", verification: "Validation", operations: "Operation" } ;
const PROJECT_ACTIVITY_STATE_LABELS    : Record<string, string> = { complete: "완료", "in-progress": "진행 중", blocked: "차단됨" }                                                                         ;
