...
export function validateObsidianDocument(input: ObsidianDocumentInput): ObsidianDocumentContract {
	...
	if (!domain
		|| !SAFE_SEGMENT.test(domain)
		|| !capability
		|| !SAFE_SEGMENT.test(capability)) problems.push(issue("PROPERTY_INVALID", "domain과 capability는 안전한 단일 경로 segment여야 합니다.", path));
	...
	if (!h1
		|| headingCapability !== capability
		|| !humanTitle
		|| LINEAR_ANYWHERE.test(`${headingCapability ?? ""} ${humanTitle ?? ""}`)
		|| MACHINE_TITLE_TOKEN.test(`${domain ?? ""} ${headingCapability ?? ""} ${humanTitle ?? ""}`)
		|| [domain, headingCapability, humanTitle].some(value => value && NUMERIC_TITLE_PREFIX.test(value))) problems.push(issue("HEADING_INVALID", "H1과 경로는 사람이 읽는 도메인·기능·제목을 사용하며 ID, 번호 접두어, [라벨]을 포함하지 않아야 합니다.", path));
