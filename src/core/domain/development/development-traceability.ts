...
export interface SpecContract {
	acceptances: AcceptanceContract[]
}
...
export function deriveVerificationVerdict(receipt: Pick<VerificationReceipt, "acceptanceCoverage" | "exceptionCoverage" | "requiredAcceptanceRefs" | "applicableExceptionStages">): VerificationReceipt["verdict"] {
	...
	if (!coverage.length
		|| !requiredAcceptances.length
		|| requiredAcceptances.some(ref => !acceptanceRefs.has(ref))
		|| requiredExceptionStages.some(({ exceptionRef, stages }) => stages.some(stage => !exceptionStages.has(`${exceptionRef}|${stage}`)))) return "partial";
