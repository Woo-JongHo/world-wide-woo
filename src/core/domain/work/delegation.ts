...
export interface NativeDelegationActivity {
	...
	readonly source      : { readonly turnId: string; readonly itemId: string } ;
}
...
interface MutableTask {
	...
	activities      : NativeDelegationActivity[] ;
	result          : string | null              ;
}
...
export function projectNativeDelegation(activities: readonly ProjectActivity[], rootThreadId?: string | null): readonly NativeDelegationProjection[] {
	...
	const projectionGroups = new Map<string, { activities: ProjectActivity[]; itemIds: Set<string> }>();
	...
	// Spawn 입양 — provisional 태스크가 아직 spawn 아이템을 못 얻었으면 그 자리에서 입양하고, 없으면 새로 만든다.
	const adoptReceiver = (id: string, activity: ProjectActivity, projectionKey: string, itemId: string): MutableTask => {
		const provisional = latest.get(id);
		if (provisional?.spawnItemId === null) { provisional.spawnItemId = itemId; provisional.projectionKey = projectionKey; return provisional; }
		return create(id, activity, itemId);
	};
	for (const activity of ordered) {
		const item = rec(rec(activity.payload.params)?.item);
		if (!item
			|| !["collabagenttoolcall", "subagentactivity"].includes(norm(item.type))
			|| !activity.nativeRefs.turnId
			|| !activity.nativeRefs.itemId) continue;
		const refs          = { turnId: activity.nativeRefs.turnId, itemId: activity.nativeRefs.itemId }            ;
		const projectionKey = `${rootThreadId ?? activity.nativeRefs.threadId ?? "unknown"}\u0000${refs.turnId}`    ;
		const group         = projectionGroups.get(projectionKey) ?? { activities: [], itemIds: new Set<string>() } ;
		group.activities.push(activity); group.itemIds.add(refs.itemId);
		projectionGroups.set(projectionKey, group);
		if (norm(item.type) === "subagentactivity") {
			...
			const message = first(item.message, item.text, item.result); task.activities.push(observation(activity, refs, item, message)); if (task.status === "completed" && message) task.result = message;
			continue;
		}
		...
		if (tool === "spawnagent") {
			...
			if (!affected || affected.length === 0 && receivers.length) {
				affected = receivers.map((id) => adoptReceiver(id, activity, projectionKey, itemId));
				spawned.set(spawnKey, affected);
			}
			for (const task of affected) enrichSpawn(task, activity, refs, item);
			continue;
		}
		for (const [id, stateValue] of Object.entries(rec(item.agentsStates) ?? {})) updateState(latest.get(id), activity, refs, item, rec(stateValue));
		if (["sendmessage", "followuptask", "sendinput"].includes(tool)) for (const id of receiversFor(item)) { const task = latest.get(id); const message = first(item.prompt, item.message, item.input); if (task && message) task.activities.push(observation(activity, refs, item, message)); }
	}
	...
	for (const activity of ordered) {
		...
		if (!threadId
			|| !turnId
			|| !itemId
			|| isReasoningActivityPayload(activity.payload)) continue;
		const item = rec(rec(activity.payload.params)?.item);
		...
		if (!attempt) continue;
		attempt.activities.push(publicChildObservation(activity, { turnId, itemId }, item));
	}

	return Object.freeze([...projectionGroups.entries()].flatMap(([key, group]) => {
		const visible = tasks.filter((task) => task.projectionKey === key && (!rootThreadId || descendant(task, tasks, rootThreadId)));
		if (!visible.length) return [];
		const [sourceThreadId = "unknown", turnId = "unknown"] = key.split("\u0000");
		return [{ sourceThreadId, turnId, activityIds: Object.freeze(group.activities.map((activity) => activity.id)), itemIds: Object.freeze([...group.itemIds]), tasks: Object.freeze(visible.map((task) => Object.freeze({ ...task, parentRef: parentRef(task, tasks), activities: Object.freeze([...task.activities].sort((a, b) => activitySequence(a.activityId, ordered) - activitySequence(b.activityId, ordered))) }))) }];
	}));
}

function enrichSpawn(task: MutableTask, activity: ProjectActivity, refs: NativeDelegationActivity["source"], item: Readonly<Record<string, unknown>>): void { task.parentId ??= txt(item.senderThreadId); task.task ??= txt(item.prompt); task.model ??= first(item.model, rec(item.settings)?.model); task.reasoningEffort ??= first(item.reasoningEffort, item.reasoning_effort, rec(item.settings)?.reasoning_effort); updateState(task, activity, refs, item, rec(rec(item.agentsStates)?.[task.id])); }
function updateState(task: MutableTask | undefined, activity: ProjectActivity, refs: NativeDelegationActivity["source"], item: Readonly<Record<string, unknown>>, state: Readonly<Record<string, unknown>> | undefined): void { if (!task) return; const status = lifecycle(state?.status); if (status !== "unknown") task.status = status; const message = first(state?.message, item.message); if (message) task.activities.push(observation(activity, refs, item, message)); if (task.status === "completed" && message) task.result = message; }
...
function publicChildObservation(activity: ProjectActivity, refs: NativeDelegationActivity["source"], item?: Readonly<Record<string, unknown>>): NativeDelegationActivity { const payload = activity.payload, params = rec(payload.params); const message = first(payload.text, payload.content, params?.text, item?.text, item?.content, item?.message, item?.output, item?.result); const kind = first(item?.tool, item?.command, payload.method, activity.kind) ?? activity.kind; return { activityId: activity.id, itemId: refs.itemId, kind, message, senderId: activity.nativeRefs.threadId ?? null, receiverIds: Object.freeze([]), attribution: "observed", source: { turnId: refs.turnId, itemId: refs.itemId } }; }
function observation(activity: ProjectActivity, refs: NativeDelegationActivity["source"], item: Readonly<Record<string, unknown>>, message: string | null): NativeDelegationActivity { return { activityId: activity.id, itemId: refs.itemId, kind: first(item.kind, item.tool) ?? "activity", message, senderId: txt(item.senderThreadId), receiverIds: Object.freeze(strings(item.receiverThreadIds)), attribution: "observed", source: { turnId: refs.turnId, itemId: refs.itemId } }; }
