...
function itemId(value: unknown): unknown {
...
function validateTodoItems(rawItems: readonly unknown[], runtime: boolean): readonly TodoItem[] {
	...
	const items = rawItems.map((raw) => {
		...
		const details = rawDetails.map((detail) => {
			...
			if (validated.status === "in_progress") activeDetails += 1;
			return Object.freeze({ id: detail.id, ...validated });
		});
		if (activeDetails > 1 && !runtime) fail("at most one todo detail may be in progress");
...
function validateTodoRelationships(items: readonly TodoItem[], source: TodoNativePlanSource | undefined): void {
	if (items.filter(item => item.status === "in_progress").length > 1 && !source) fail("at most one manual todo item may be in progress");
...
export function parseTodoMarkdown(markdown: string): TodoDocument {
	...
	if (!titleLine.startsWith("# ") || titleLine.slice(2).length === 0) fail("invalid todo heading");
	...
	return validateTodoDocument({ ...header, title: titleLine.slice(2), items });
}
...
export function patchTodoMarkdown(markdown: string, next: TodoDocument): string {
	...
	const titleLine  = lines[1]                     ;
	if (!headerLine || !titleLine) fail("invalid todo markdown layout");
	headerLine.content = `<!-- ${renderHeader(todo)} -->`;
	titleLine.content = `# ${todo.title}`;
	...
	for (let index = managed.length - 1; index >= desired.length; index -= 1) {
		const lineIndex = managed[index];
		if (lineIndex === undefined) fail("invalid managed todo range");
		lines.splice(lineIndex, 1);
	}
	if (desired.length > managed.length) {
		const lastManaged = managed.at(-1)                                                          ;
		const insertion   = lastManaged === undefined ? Math.min(3, lines.length) : lastManaged + 1 ;
...
function isLineEnding(value: string | undefined): value is MarkdownLine["eol"] {
	return value === "\r\n" || value === "\n" || value === "";
}
...
function parseItemLine(line: string): TodoItem {
	...
	const [, checkbox, rawContent, comment] = match;
	if (checkbox === undefined || rawContent === undefined || comment === undefined) fail("invalid todo item markdown");
	const metadata = parseComment(comment);
	if (!isRecord(metadata)
		|| !hasExactKeys(metadata, ["id", "status", "evidenceIds"], ["source"])
	...
	return {
		...
		evidenceIds : validateEvidenceIds(metadata.evidenceIds),
		content,
		details: [],
		...(metadata.source === undefined ? {} : { source: validateTodoItemSource(metadata.source) }),
	};
}
...
function validateTodoEntry(raw: Record<string, unknown>, label: string): Pick<TodoDetail, "content" | "status" | "evidenceIds"> {
	...
	if (!isTodoItemStatus(raw.status)) fail(`invalid ${label} status`);
	return { content, status: raw.status, evidenceIds: validateEvidenceIds(raw.evidenceIds) };
}

function isTodoItemStatus(value: unknown): value is TodoItemStatus {
	return typeof value === "string" && TODO_ITEM_STATUSES.some(status => status === value);
}

function validateEvidenceIds(value: unknown): readonly string[] {
	if (!Array.isArray(value)
		|| value.length > MAX_TODO_EVIDENCE
		|| new Set(value).size !== value.length
		|| value.some(id => typeof id !== "string" || !idPattern.test(id))) fail("invalid evidence ids");
	return Object.freeze(value.map(id => String(id)));
}
...
function validatePlanRevision(value: unknown): TodoPlanRevisionReference {
	...
	if (!isSha256Hex(value.sourceRevisionKeyDigest)
		|| !isOpaqueReference(value.activityId)
		|| !isPositiveInteger(value.sequence)
		|| !isSourceDigest(value.sourceDigest)) fail("invalid Plan revision reference");
	return Object.freeze({
...
function validateExecutionReference(value: unknown): TodoExecutionReference {
	...
	const { provider, model, agentId, threadId, runId } = value;
	for (const field of [provider, model, agentId, threadId, runId]) {
		if (field !== null && !isOpaqueReference(field)) fail("invalid execution reference");
	}
	return Object.freeze({
		provider : validatedReference(provider),
		model    : validatedReference(model),
		agentId  : validatedReference(agentId),
		threadId : validatedReference(threadId),
		runId    : validatedReference(runId),
	});
}

function validatedReference(value: unknown): string | null {
	if (value === null || isOpaqueReference(value)) return value;
	return fail("invalid execution reference");
}
...
function isOpaqueReference(value: unknown): value is string {
	return (
		...
		&& value.length <= 512
		&& value.trim() === value
		&& !referenceControlPattern.test(value)
	);
}
