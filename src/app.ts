...
export interface RunAppOptions { resumeThreadId?: string; executionLane?: ExecutionLane; design?: "astra"; runtimeConfig?: string }
export async function runApp(options: RunAppOptions = {}): Promise<void> {
	...
	const requestCapabilityFactory = options.runtimeConfig ? await loadRequestCapabilityConfig(options.runtimeConfig) : undefined;
	...
	try {
		const { createProjectAuthController } = await import("./adapters/outbound/authentication/project-auth"); runProjectWorkbenchShell({
			workbench: project.workbench, cwd: project.workspace.root, usage: project.usage, auth: createProjectAuthController(), design: options.design,
