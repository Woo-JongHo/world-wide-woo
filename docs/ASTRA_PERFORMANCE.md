...
않았다. 실제 `AstraWorkspace + renderLayoutFrame`에서 수정 전 `renderedBlocks`
...
반환해 내용 최대폭을 재는 dense auto-size 부모와 잠재적으로 다르다. Astra의 현재
...
1. **첫 표시·resize·장기 기록:** `AstraTranscriptView`, `ChatScrollView`, layout 경계에 message/block별 row-count index와 visible-range 렌더링 계약을 설계한다. contentHeight, scrollTop, anchor, follow-end, scrollbar를 유지해야 한다. 단순 slice나 history 삭제는 금지한다. 현재 측정으로 검토 트리거는 충족됐으며, 별도 viewport 변경안에서 행동 테스트와 baseline부터 만든다.
...
bun run check
bun test
ASTRA_BENCH_REPS=5 bun scripts/astra-render-benchmark.ts
ASTRA_BENCH_REPS=50 ASTRA_BENCH_OUTPUT=/tmp/astra-benchmark.json bun scripts/astra-render-benchmark.ts
ASTRA_BENCH_COUNTS=5000 ASTRA_BENCH_REPS=5 ASTRA_BENCH_BUDGET_MS=180000 bun scripts/astra-render-benchmark.ts
```
