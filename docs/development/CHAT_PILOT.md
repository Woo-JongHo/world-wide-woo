...
Linear의 Chat 관련 12개 이슈 본문과 코멘트를 조회했다. 모두 코멘트 0개였으며, 진행 기록은 이슈 본문의 “개발 현황”과 원래 로컬 작업 트리의 Evidence에 있었다. 아래 표는 **기존 기록에서 확인한 남은 요구**이고 이 PR에서 제품 코드를 새로 검증한 결과가 아니다.
...
4. Chat의 근거 이동·# T-note·Composer와 Todo·Tracer를 연결하고 화면 가독성·왕복 흐름을 검증한다.
5. WOO-692에서 실제 Native 연결·40/80/120열·무채색·resize/focus 시나리오를 수락한다.
