...
Astra는 현재 `WorkbenchSnapshot`과 연결된 `WWW Dashboard`에서 시작한다. 세션 상태, 현재 작업, 목표, 대화·계획·Todo 수, 승인·오류와 Linear 연결 상태를 먼저 읽는다. 입력란은 바로 사용할 수 있고, 실행 타임라인은 `Ctrl+G → 1` 또는 `/chat`으로 연다.
...
- 본문: 요청 → 도구 실행 → 공개 응답 → 질문 요약이 기록 순서대로 이어지는 ZChat 타임라인. 요청 청록, 응답 라일락, 도구 블루, 계획 골드, 질문 요약 코랄의 제목 표식만 사용해 본문과 구분한다. 실행 중·실패·펼친 Bash에만 명령과 출력의 경계를 둔다. 완료된 명령은 한 줄로 접고 실행 중 출력은 마지막 6줄을 미리 보여준다.
- 넓은 화면: 112열 이상이고 기본 입력 상태의 본문 높이가 충분하면 계획을 오른쪽에 함께 표시한다. `Ctrl+B`로 사이드바를 닫거나 다시 열 수 있고 화면 이동 중에도 선택을 유지한다. 입력·자동완성이 늘어나도 계획 열의 폭은 유지하고, 세로로는 스크롤 가능한 본문을 줄인다. 질문 요약은 별도 보조 패널이 아니라 ZChat 안에서 해당 질문의 마지막 근거 뒤에 표시된다. 전체 계획은 Ctrl+G 2에서 읽는다.
- 좁은 화면: 계획 보조 영역을 숨기고 실행·입력·HUD를 유지한다. 핵심 조작 기준은 80×24다.
- 입력과 HUD: 입력칸의 위·아래 경계와 `›`로 포커스를 표시한다. HUD는 `A›` 캐릭터가 있는 모델·권한 행, 현재 행동 행, provider 잔여 행의 최대 세 줄이다. Codex·Claude·Gemini·Z.AI 이름은 각각 고유한 저채도 색으로 항상 남기고, 폭이 부족하면 값과 리셋 시각만 축약한다. 한도는 청구 금액이 아니며 `*`는 마지막 성공 값이다.
...
Mac의 **Control** 키를 사용한다(Command가 아니다). Ctrl+G를 누른 뒤 손을 떼고 숫자를 누른다. 선택기에서 ↑↓와 Enter도 가능하며 작성 중인 초안은 유지된다. F2–F8은 기존 사용자를 위한 보조 키로 남긴다. 움직임을 줄이려면 `ASTRA_REDUCED_MOTION=1 bun start astra`를 사용한다. `NO_COLOR` 환경에서도 움직이는 색 효과를 사용하지 않는다.
...
완료된 질문마다 T-note 생성 큐가 별도로 동작한다. 질문·왜·결과 저장 계약은 유지하고 다음 실행을 막지 않는다. Astra에서는 이를 `## 질문 제목`, `## 원인`, `## 결과` 구조로 읽기 좋게 표시한다. 완성된 요약은 별도 화면이나 명령으로 탐색하게 하지 않고 ZChat 타임라인에서 해당 질문의 근거 뒤에 표시한다. 기존 `/tnotes`와 `/tnote` 파서는 호환을 위해 남지만 Astra의 화면 이동·명령 검색에서는 노출하지 않는다. 생성 실패는 자동 생성 보류로 표시하며, 관측하지 않은 요약을 대신 만들지 않는다.
