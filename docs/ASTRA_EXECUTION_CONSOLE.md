...
- 넓은 화면: 112열 이상이고 기본 입력 상태의 본문 높이가 충분하면 계획을 오른쪽에 함께 표시한다. `Ctrl+B`로 사이드바를 닫거나 다시 열 수 있고 화면 이동 중에도 선택을 유지한다. 입력·자동완성이 늘어나도 계획 열의 폭은 유지하고, 세로로는 스크롤 가능한 본문을 줄인다. 질문 요약은 ZChat 안에서 해당 질문의 마지막 근거 뒤에 하나의 테두리 박스로 표시한다. 질문 제목·Reason·Proposal·Action·Result·근거 링크를 함께 묶고, 항목명은 청회색·금색·파랑·민트로 구분하며 본문은 흰색을 유지한다. 전체 계획은 Ctrl+G 2에서 읽는다.
...
완료된 질문마다 T-note 생성 큐가 별도로 동작한다. 새 T-note는 `질문 · Reason · Proposal · Action · Result` 종료 보고서로 저장하고 다음 실행을 막지 않는다. Astra에서는 질문 제목 아래 네 구획을 왼쪽 정렬 Markdown으로 표시한다. `Action`은 실제 수행만, `Result`는 코드·문서·GitHub·Linear의 관측된 변경 상태를 담는다. 한 Turn의 source activity가 안전 한도 100개를 넘으면 전체 range와 질문·시작·최종 응답·완료 경계를 유지한 채 중간 활동을 결정론적으로 균등 표본화한다. 저장된 옛 `질문 · 왜 · 결과` 노트는 `## 원인`, `## 결과` 구조로 계속 읽을 수 있다. 완성된 보고서는 별도 화면이나 명령으로 탐색하게 하지 않고 ZChat 타임라인에서 해당 질문의 근거 뒤에 표시한다. 기존 `/tnotes`와 `/tnote` 파서는 호환을 위해 남지만 Astra의 화면 이동·명령 검색에서는 노출하지 않는다. 생성 실패는 자동 생성 보류로 표시하며, 관측하지 않은 내용을 대신 만들지 않는다.
