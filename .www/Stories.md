...
- ST-012-14 | EP-012 | Stats 정보 구조와 효율 지표 계약을 확정한다 | 대표 Product·RPA·Coding session 사례에서 사용자가 판단해야 할 질문, 지표 정의, source authority, 계산식, 표시 우선순위와 non-goal을 문서화하고 사람 승인을 받는다. token 절감만을 효율로 정의하지 않으며 관측할 수 없는 값은 unknown으로 남긴다.
- ST-012-15 | EP-012 | 세션 활동을 Stats projection으로 계산한다 | Activity journal·usage·Todo·T-note·approval·Evidence reference를 복제하지 않고 읽어 session 시간, turn, agent, tool, approval, failure, token 지표를 결정론적으로 계산한다. 같은 snapshot은 같은 결과를 내고 누락 source는 명시적으로 unknown이다.
- ST-012-16 | EP-012 | 목적·행동·결과 세션 요약을 근거와 함께 제공한다 | Stats가 Session Goal, 관측된 주요 행동, durable result와 Evidence를 목적·행동·결과로 구분한다. 모델의 hidden reasoning이나 관측되지 않은 성공을 생성하지 않고 각 항목에서 source activity 또는 artifact로 이동할 수 있다.
