import os,pty,subprocess,fcntl,termios,struct,time,select,json,signal,codecs,re
from pathlib import Path
root=Path.cwd(); out=root/'.www/scratchpad/2026-09-07-stats-enter-pty';out.mkdir(exist_ok=True)
...
class RawScreen:
 def __init__(self): self.columns=80;self.lines=36;self.display=[]
 def resize(self,lines,columns): self.lines=lines;self.columns=columns
screen=RawScreen();decoder=codecs.getincrementaldecoder('utf-8')('replace');record=bytearray();steps=[];failed=False
def state():
