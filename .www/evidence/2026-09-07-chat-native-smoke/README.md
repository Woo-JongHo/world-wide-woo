...
- 문서 날짜는 Asia/Seoul(KST), JSON recordedAt은 UTC다.
...
[Chat 관측](native-chat-probe.json)은 실제 Native → ProjectWorkbench → WorkbenchChatView 경로를 실행했다. 사용자 입력은 snapshot 전 과정에서 최대 1개였고 live draft가 관측됐고 최종 snapshot에는 완료 답변 하나가 있었다. 이 JSON은 중간 전이 시각을 보존하지 않는다. 한글·emoji 답변을 40·80·120열로 렌더링했으며 세 폭 모두 overflow가 0이었다. 각 폭의 실제 plain rows와 최종 Chat identity가 JSON에 있다.

실행 소스는 [어댑터 probe](native-refs-probe.ts), [Chat probe](native-chat-probe.ts)에 보존한다. 프로젝트 root에서 Bun으로 실행할 수 있으며 실제 모델 호출과 새 임시 세션을 만든다. 소스는 날짜 접두어가 있는 JSON 파일에 결과를 쓰며, 첨부 JSON은 최초 실행 결과를 짧은 이름으로 복사한 것이다. 각 실행의 새 결과는 별도 기록으로 보존해야 한다.

표시된 `실행 기록 0개`는 도구/실행 카드의 집계이며 전체 activity journal 건수를 뜻하지 않는다. 이 표본은 도구 사용을 요청하지 않았다.
