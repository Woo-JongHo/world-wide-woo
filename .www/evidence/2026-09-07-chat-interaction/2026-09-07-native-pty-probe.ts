...
const evidenceOutputDir = process.env.WOO_EVIDENCE_OUTPUT_DIR ?? join(import.meta.dir, `replay-direct-${Date.now()}`);
mkdirSync(evidenceOutputDir, { recursive: true });
writeFileSync(join(evidenceOutputDir,"2026-09-07-native-pty-events.jsonl"), "");
...
wb.subscribe(snapshot => {
 writeFileSync(join(evidenceOutputDir,"2026-09-07-native-pty-state.json"), JSON.stringify({phase:snapshot.phase,threadId:snapshot.threadId,activeTurnId:snapshot.activeTurnId,draft:!!snapshot.draft,chat:snapshot.chat,error:snapshot.error,selectedActivityId:snapshot.selectedActivityId}));
});
