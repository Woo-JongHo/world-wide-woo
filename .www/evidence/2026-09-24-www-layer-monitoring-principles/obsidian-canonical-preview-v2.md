...
현재 Runtime Monitor와 Cache telemetry, 전체 Context 점유율 projection이 존재한다. Context 점유율은 동일 크기 cell로 표시하며 관련 UI 테스트 69건이 통과했다. Figma에는 7계층과 Golden Signals 기준 화면 node 77:2가 생성됐다.

### Partial

Cache의 일부 계층만 실제 source가 연결되어 unobserved가 남는다. Figma 기준 화면의 예시 수치는 구현 telemetry가 아니라 reference다.

### Gap

| GAP-ID | 관련 Contract | 내용 | Linear |
|---|---|---|---|
| GAP-MON-001 | INV-001/002 | 7계층 trace와 wait/work boundary가 아직 없다. | WOO-913 |
| GAP-MON-002 | INV-003 | traffic/error/saturation window aggregate와 percentile source가 없다. | WOO-913 |
