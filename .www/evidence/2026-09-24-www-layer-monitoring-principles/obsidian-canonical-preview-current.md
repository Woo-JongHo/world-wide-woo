...
Core의 bounded LayerPerformanceRecorder가 7계층 wait/work, partial, failure, monotonic order와 p50·p95·p99 window projection을 소유한다. Native Receive, Event Queue, State Projection, Snapshot Publish, Render Schedule, Layout / Materialize, Terminal Write의 실제 경계를 이벤트별 trace로 연결했다. /monitor는 현재 trace의 wait/work를, /dashboard는 window trace·error와 가장 느린 p95 계층을 표시한다. Context 점유율은 동일 크기 cell로 표시하고 새 turn에서 이전 점유값을 무효화한다. Figma에는 기준 화면 node 77:2가 생성됐다.

### Partial

장기 persistence와 freshness 상태, 독립 saturation source, 계측 오버헤드 budget은 아직 없다. Cache 7종은 실제 source가 보고한 값만 표시하며 source가 없는 항목은 의도적으로 unobserved다. Figma 기준 화면의 예시 수치는 구현 telemetry가 아니라 reference다.

### Gap

| GAP-ID | 관련 Contract | 내용 | Linear |
|---|---|---|---|
| GAP-MON-001 | INV-003 | 독립 saturation·freshness source와 장기 window를 추가해야 한다. | WOO-913 |
| GAP-MON-002 | AC-MON-004 | telemetry overhead budget을 실측해 결정해야 한다. | WOO-913 |
