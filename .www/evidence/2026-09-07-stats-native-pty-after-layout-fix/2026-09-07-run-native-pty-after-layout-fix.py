...
import pyte
root=Path.cwd(); out=root/'.www/scratchpad/2026-09-07-native-pty-after-layout-fix';out.mkdir(exist_ok=True)
...
screen=pyte.Screen(80,36);stream=pyte.Stream(screen);decoder=codecs.getincrementaldecoder('utf-8')('replace');record=bytearray();steps=[];failed=False
def state():
