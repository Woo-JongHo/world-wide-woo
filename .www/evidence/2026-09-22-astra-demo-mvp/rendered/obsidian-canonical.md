...
### Scenario DB-006 · 합성 MVP 화면을 안전하게 순회한다

**Given** 사용자가 실제 Workbench 세션에서 `/demo`를 입력했다.

**When** Demo가 활성화되어 R/E로 Chat·Dashboard·Usage·Context·Cache·Workflow·Plan을 순환하고 Esc로 종료한다.

**Then** 모든 화면은 DEMO DATA provenance가 붙은 합성값만 표시하며 Workbench command를 dispatch하지 않는다. Demo 중 live snapshot과 usage는 화면과 분리해 계속 보관하고, Esc 뒤 최신 live snapshot·usage·pending approval과 진입 전 화면을 복구한다. 조회용 스크롤 키는 유지한다.
...
### INV-006 · Demo는 live state와 쓰기를 격리한 session-only presentation이다

Demo snapshot은 live WorkbenchSnapshot을 복사하거나 상속하지 않고 명시적인 합성 필드만 구성한다. Demo의 R/E·스크롤·Esc 입력은 외부 dispatch를 만들지 않는다. 합성 token·quota·cache 값은 내부 합계와 분모가 일치해야 하고 모든 화면에서 DEMO DATA로 식별 가능해야 한다.
...
| From | Event / Condition | To | 비고 |
|---|---|---|---|
| live | `/demo` | demo | live snapshot·usage와 진입 화면을 별도 보관 |
| demo | R / E | demo | 7개 합성 화면을 순환; 쓰기 없음 |
| demo | Esc | live | 가장 최근 live snapshot·usage·approval UI를 복구 |

Demo는 Workbench domain phase를 바꾸지 않는 shell-local presentation state다.
...
/demo
  ↓
명시적 synthetic WorkbenchSnapshot + UsageSnapshot
  ↓                         live subscription/polling
7개 Astra 화면              → 별도 live buffer
  ↓ Esc
최신 live snapshot·usage·approval UI 복구
```

합성 snapshot은 실제 Linear dashboard, reasoning, execution receipt, Todo, T-note 또는 agent detail을 입력으로 받지 않는다.
...
| EXC-WORKBENCH-004 | Demo에 live 업무 내용이 섞이거나 Demo 조작이 실제 command를 dispatch함 | synthetic-only fixture와 shell dispatch assertion | Demo provenance 유지, live 값 비노출 | Esc로 최신 live snapshot과 approval UI 복구 | 실제 세션 state와 입력 이력 |
...
| AC-WORKBENCH-006 | `/demo`는 7개 MVP 화면을 R/E로 순환하고 스크롤을 허용하며, 합성값만 표시하고 dispatch 없이 Esc 뒤 최신 live state와 approval UI를 복구한다. | TEST-WORKBENCH-007 | test/astra-shell.test.ts, .www/scratchpad/astra-demo-independent-review.md | PASS |
...
| TEST-WORKBENCH-007 | Demo 7화면 순환, provenance, live 비노출, 무 dispatch, 스크롤, Esc 복구와 approval 동기화 | TUI integration / independent read-only review | AC-WORKBENCH-006, INV-006, EXC-WORKBENCH-004 |
...
| Demo synthetic snapshot·usage fixture | src/adapters/inbound/tui/features/demo/astra-demo.ts | — (stable Code-ID not assigned) |
| Demo command·navigation·live restore | src/adapters/inbound/tui/shell/workbench-shell.ts | — (stable Code-ID not assigned) |
...
- `/demo` session-only presentation은 7개 MVP 화면, R/E 순환, 읽기 스크롤, Esc live 복구를 구현했다.
- Demo fixture는 live snapshot을 상속하지 않고 합성 데이터만 명시하며 실제 reasoning·Linear·receipt를 노출하지 않는다.
...
#### DEC-WORKBENCH-004 · Demo를 live state와 분리된 session-only presentation으로 둔다
...
`/demo`는 저장·외부 쓰기·Workbench dispatch가 없는 shell-local presentation이다. 실제 snapshot을 합성값으로 덮어쓰지 않고 별도 live buffer와 별도 synthetic snapshot을 유지한다. R/E는 고정된 7개 MVP 화면을 순환하고 Esc는 가장 최근 live snapshot·usage·approval UI와 진입 화면을 복구한다.

**Why**

Figma 정보 구조를 실제 계측 준비도와 독립적으로 검토할 수 있어야 하지만 합성값이 실제 업무 내용이나 telemetry로 오인되어서는 안 된다.

**Alternatives**

live snapshot 위에 일부 필드만 덮는 방식은 실제 Linear·reasoning 필드가 노출되어 거절했다. Demo를 별도 영속 세션으로 만드는 방식은 복구·identity·쓰기를 불필요하게 확대해 채택하지 않았다.

**Impact**

새 WorkbenchSnapshot 필드가 추가돼도 Demo에 자동 상속되지 않는다. Demo 사용 중 live event는 폐기하지 않고 복귀용 buffer에만 갱신한다.

### 2026-09-22 Demo Evidence

| Date | Evidence | Covers |
|---|---|---|
| 2026-09-22 | test/astra-shell.test.ts | TEST-WORKBENCH-007 |
| 2026-09-22 | .www/scratchpad/astra-demo-independent-review.md | synthetic 격리·복구·스크롤 독립 재검토 |
...
| 2026-09-22 | `/demo`의 synthetic-only 7화면 순환과 live 복구 계약을 추가 | Figma MVP를 실제 telemetry와 혼동하지 않고 한 세션에서 검토하기 위해 |
