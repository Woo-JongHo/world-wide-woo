...
const repetitions = Number(process.env.WWW_BENCH_REPS ?? 50);
assert(counts.every(n => Number.isInteger(n) && n >= 1 && n <= 10000));
assert(Number.isInteger(repetitions) && repetitions >= 5 && repetitions <= 500);
const configuredResizeBudget = process.env.WWW_BENCH_RESIZE_BUDGET_MS;
...
const sourceRevision = process.env.WWW_BENCH_SOURCE_HASH ?? "not-provided" ;
const began          = performance.now()                                   ;
const budgetMs       = Number(process.env.WWW_BENCH_BUDGET_MS ?? 120000)   ;
...
function bodyWorkspace(get: () => ReturnType<typeof fixture>) {
  ...
  return new WwwWorkspace(
    ...
    null,
    undefined,
    new WwwExecutionHeading(get, undefined, clock, false),
  );
}
...
function metricDelta(after: ReturnType<WwwWorkspace["transcript"]["cacheMetrics"]>, before?: ReturnType<WwwWorkspace["transcript"]["cacheMetrics"]>) {
...
function runFocusedVerification() {
  ...
  const inset            = new WwwInset(child)                                                                                       ;
  inset.render(80);
...
for (const count of counts) {
  ...
  try {
    runProjectWorkbenchShell({ surface: "www", terminal, cwd: "/benchmark/www", workbench,
    ...
    await tick();
    assert(terminal.output.includes("www"), "production shell did not paint");
...
if (process.env.WWW_BENCH_OUTPUT) await writeFile(process.env.WWW_BENCH_OUTPUT, JSON.stringify(report, null, 2) + "\n");
